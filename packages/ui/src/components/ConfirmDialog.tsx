...
export function ConfirmDialog({
  ...
  const confirmVariant = variant === 'danger' ? 'danger' as const : variant === 'warning' ? 'secondary' as const : 'primary' as const;

  return (
    <Modal open={open} onClose={onCancel}>
      <div className="text-center">
        <span className="text-4xl">{variant === 'danger' ? '🗑️' : variant === 'warning' ? '⚠️' : '❓'}</span>
