...
export function BeautyExpertTalkCard({ talk, onRegister, className = '' }: BeautyExpertTalkCardProps): JSX.Element {
  return (
    <div className={cn('rounded-2xl border border-indigo-100 bg-white p-4 dark:border-indigo-900 dark:bg-gray-900', className)}>
      <div className="flex items-start gap-3">
        ...
        <div className="min-w-0 flex-1">
          ...
          <p className="text-[10px] text-text-tertiary dark:text-gray-500">🎙️ {talk.expert} · 📅 {talk.date}</p>
        </div>
        {talk.isFree && <span className="shrink-0 rounded-full bg-emerald-50 px-2 py-0.5 text-[10px] font-bold text-emerald-700 dark:bg-emerald-950 dark:text-emerald-300">مجاني</span>}
      </div>

      {talk.seats !== undefined && (
        <div className="mt-2 rounded-lg bg-indigo-50 p-2 text-center dark:bg-indigo-950">
          <p className="text-[10px] text-indigo-700 dark:text-indigo-300">🎟️ {talk.seats} مقعد متبقي</p>
        </div>
      )}

      <button type="button" onClick={onRegister} className="mt-3 w-full rounded-xl bg-indigo-600 py-2 text-xs font-bold text-white hover:bg-indigo-700 active:scale-[0.98] transition-all">
        سجلي حضوركِ 🎤
      </button>
    </div>
  );
