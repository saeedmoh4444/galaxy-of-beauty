  ...
  return (
    ...
    >
      ...
      <div className="mt-3 grid grid-cols-2 gap-1.5">
        {TIPS.map((t) => (
          <div key={t.title} className="rounded-lg bg-fuchsia-50 px-2.5 py-2 dark:bg-fuchsia-950">
            <span className="text-sm">{t.emoji}</span>
            <p className="mt-0.5 text-[10px] font-bold text-fuchsia-800 dark:text-fuchsia-200">
              {t.title}
            </p>
            <p className="text-[9px] text-fuchsia-600 dark:text-fuchsia-400">{t.desc}</p>
          </div>
        ))}
      </div>
    </div>
