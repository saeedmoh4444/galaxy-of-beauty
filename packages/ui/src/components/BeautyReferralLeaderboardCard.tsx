...
interface Leader { name: string; referrals: number; emoji?: string; }

interface BeautyReferralLeaderboardCardProps { leaders: Leader[]; userRank?: number; className?: string; }

export function BeautyReferralLeaderboardCard({ leaders, userRank, className = '' }: BeautyReferralLeaderboardCardProps): JSX.Element | null {
    ...
    <div className={cn('rounded-2xl border border-amber-100 bg-white p-4 dark:border-amber-900 dark:bg-gray-900', className)}>
      ...
      <div className="mt-3 space-y-1">
        {leaders.slice(0, 5).map((l, i) => (
          <div key={l.name} className={cn('flex items-center gap-2 rounded-lg px-3 py-2', i === 0 ? 'bg-amber-50 dark:bg-amber-950' : 'bg-gray-50 dark:bg-gray-800')}>
            <span className="text-sm w-6 text-center">{medals[i] || `${i + 1}.`}</span>
            <span className="text-sm">{l.emoji || '👩'}</span>
            <span className="flex-1 text-[10px] font-bold text-text-primary dark:text-gray-100">{l.name}</span>
            <span className="text-[10px] font-bold text-amber-700 dark:text-amber-300">{l.referrals} إحالة</span>
          </div>
        ))}
      </div>
