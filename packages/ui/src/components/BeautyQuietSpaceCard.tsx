...
export function BeautyQuietSpaceCard({ features, className = '' }: BeautyQuietSpaceCardProps): JSX.Element | null {
  ...
  return (
    <div className={cn('rounded-2xl border border-purple-100 bg-gradient-to-br from-purple-50 to-indigo-50 p-5 dark:border-purple-900 dark:from-purple-950 dark:to-indigo-950', className)}>
      <div className="text-center">
        ...
        <h4 className="mt-1 text-sm font-bold text-purple-800 dark:text-purple-200">المساحة الهادئة</h4>
      ...
      <div className="mt-3 grid grid-cols-2 gap-1.5">
        {features.map((f) => {
          ...
          return def ? (
            <div key={f} className="flex items-center gap-2 rounded-lg bg-white/60 px-2.5 py-2 dark:bg-gray-800/60">
              <span className="text-sm">{def.emoji}</span>
              <span className="text-[10px] font-medium text-purple-800 dark:text-purple-200">{def.label}</span>
            </div>
          ) : null;
        })}
      </div>
      <p className="mt-2 text-center text-[9px] text-purple-500 dark:text-purple-400">🧘 خذي لحظة لنفسكِ</p>
    </div>
  );
}
