...
export function BeautySecretSantaCard({ group, budget, participants, onJoin, onReveal, className = '' }: BeautySecretSantaCardProps): JSX.Element {
  const isRevealed = false; // Would be date-gated in production

  return (
    <div className={cn('rounded-2xl border border-red-100 bg-gradient-to-br from-red-50 to-green-50 p-5 dark:border-red-900 dark:from-red-950 dark:to-green-950', className)}>
      ...
      {!isRevealed ? (
        <button type="button" onClick={onJoin} className="mt-3 w-full rounded-xl bg-red-600 py-2.5 text-xs font-bold text-white hover:bg-red-700 active:scale-[0.98] transition-all">
          انضمي للعبة 🎁
        </button>
      ) : (
        <button type="button" onClick={onReveal} className="mt-3 w-full rounded-xl bg-emerald-600 py-2.5 text-xs font-bold text-white hover:bg-emerald-700 active:scale-[0.98] transition-all">
          اكشفي هديتكِ 🎁
        </button>
      )}

      <p className="mt-2 text-center text-[9px] text-text-tertiary dark:text-gray-500">🎁 الهدية الأجمل هي التي تأتي من القلب</p>
    </div>
  );
}
