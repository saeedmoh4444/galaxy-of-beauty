'use client';
import type { JSX } from 'react';
import { cn } from '@galaxy/shared';
export function BeautySunscreenCard({
  className = '',
  locale = 'ar',
  title = 'دليل واقي الشمس',
  subtitle = 'كل ما تحتاجين معرفته',
}: {
  className?: string;
  locale?: 'ar' | 'en';
  title?: string;
  subtitle?: string;
}): JSX.Element {
  return (
    <div
      className={cn(
        'rounded-2xl border border-yellow-100 bg-surface-elevated p-4 dark:border-yellow-900',
        className,
      )}
    >
      <div className="flex items-center gap-2">
        <span className="text-xl">🧴</span>
        <div>
          <h4 className="text-sm font-bold text-yellow-700 dark:text-yellow-300">{title}</h4>
          <p className="text-[10px] text-yellow-800 dark:text-yellow-400">{subtitle}</p>
        </div>
      </div>
      <div className="mt-3 space-y-1">
        {[
          {
            emoji: '🌞',
            text: { ar: 'SPF 30 كحد أدنى — SPF 50 أفضل', en: 'SPF 30 minimum — SPF 50 is better' },
          },
          {
            emoji: '⏰',
            text: {
              ar: 'جددي كل ساعتين — خاصة في الصيف',
              en: 'Reapply every 2 hours — especially in summer',
            },
          },
          {
            emoji: '🥄',
            text: {
              ar: 'كمية كافية — ملعقة شاي للوجه',
              en: 'Enough amount — a teaspoon for the face',
            },
          },
          {
            emoji: '⛅',
            text: {
              ar: 'حتى في الغيوم — الأشعة تخترق',
              en: 'Even on cloudy days — rays penetrate',
            },
          },
        ].map((t, i) => (
          <div
            key={i}
            className="flex items-center gap-2 rounded-lg bg-yellow-50 px-3 py-2 dark:bg-yellow-950"
          >
            <span className="text-sm shrink-0">{t.emoji}</span>
            <span className="text-[10px] text-yellow-800 dark:text-yellow-200">
              {t.text[locale]}
            </span>
          </div>
        ))}
      </div>
    </div>
  );
}
