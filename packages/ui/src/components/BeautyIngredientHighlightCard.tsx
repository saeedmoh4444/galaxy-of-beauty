...
interface Ingredient { name: string; emoji?: string; origin: string; benefits: string[]; funFact?: string; }

interface BeautyIngredientHighlightCardProps { ingredient: Ingredient; className?: string; }

export function BeautyIngredientHighlightCard({ ingredient, className = '' }: BeautyIngredientHighlightCardProps): JSX.Element {
  return (
    <div className={cn('rounded-2xl border border-amber-100 bg-white p-4 dark:border-amber-900 dark:bg-gray-900', className)}>
      <div className="flex items-center gap-2">
        ...
        <div>
          ...
          <p className="text-[10px] text-amber-500 dark:text-amber-400">{ingredient.name} · {ingredient.origin}</p>
        </div>
      </div>
      ...
      <div className="mt-3 rounded-xl bg-amber-50 p-3 dark:bg-amber-950">
        ...
        <div className="mt-1 flex flex-wrap gap-1">
          {ingredient.benefits.map((b) => (
            <span key={b} className="rounded-full bg-white px-2 py-0.5 text-[9px] text-amber-700 dark:bg-gray-800 dark:text-amber-300">{b}</span>
          ))}
        </div>
      </div>
