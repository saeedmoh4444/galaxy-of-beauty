    ...
    return (
      <div className={cn('rounded-2xl border border-teal-100 bg-white p-5 text-center dark:border-teal-900 dark:bg-gray-900', className)}>
        ...
        <p className="text-[10px] text-text-tertiary dark:text-gray-400 mt-1">
          {score === QUESTIONS.length ? 'ممتاز! أنتِ خبيرة جمال 👑' : score >= 2 ? 'جيد! واصلي التعلم 🌟' : 'لا بأس — تعلمي المزيد 🌱'}
        </p>
        <button
          type="button"
    ...
    <div className={cn('rounded-2xl border border-teal-100 bg-white p-4 dark:border-teal-900 dark:bg-gray-900', className)}>
      ...
      <div className="mt-2 space-y-1.5">
        {q.options.map((opt, i) => {
          ...
          if (selected !== null) {
            if (i === q.correct) bg = 'bg-emerald-100 border-emerald-400 dark:bg-emerald-950 dark:border-emerald-600';
            else if (i === selected) bg = 'bg-rose-100 border-rose-400 dark:bg-rose-950 dark:border-rose-600';
          ...
          return (
            <button
              ...
              className={cn('w-full rounded-xl border-2 border-transparent px-3 py-2.5 text-left text-xs transition-all', bg)}
            >
      ...
      {selected !== null && (
        <div className="mt-2 rounded-lg bg-teal-50 p-2.5 dark:bg-teal-950">
          <p className="text-[10px] text-teal-800 dark:text-teal-200">
            {isCorrect ? '✅ صحيح! ' : '❌ خطأ! '}{q.explanation}
          </p>
        </div>
      )}
