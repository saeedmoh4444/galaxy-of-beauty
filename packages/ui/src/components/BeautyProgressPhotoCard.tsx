    ...
    >
      <div className="flex items-center justify-between">
        ...
        <button
          type="button"
          onClick={onAddPhoto}
          className="rounded-lg bg-indigo-100 p-1.5 text-indigo-600 hover:bg-indigo-200 dark:bg-indigo-950 dark:text-indigo-400"
        >
          <svg
            className="h-4 w-4"
            fill="none"
      ...
      <div className="mt-3 flex gap-2 overflow-x-auto pb-2">
        {photos.map((p, i) => (
          <div
            key={i}
            className="flex-shrink-0 w-24 rounded-xl bg-indigo-50 p-2 text-center dark:bg-indigo-950"
          >
            <span className="text-2xl">{p.emoji}</span>
            <p className="mt-1 text-[9px] font-bold text-indigo-700 dark:text-indigo-300">
              {p.date}
            </p>
