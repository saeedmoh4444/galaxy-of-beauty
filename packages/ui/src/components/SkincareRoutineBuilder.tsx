...
export function SkincareRoutineBuilder({
  ...
  if (step === 'result' && skinType) {
    ...
    return (
      <div className={cn('rounded-2xl border border-pink-100 bg-white p-5 dark:border-pink-900 dark:bg-gray-900', className)}>
        <div className="text-center">
          ...
          <p className="text-[10px] text-pink-500 dark:text-pink-400">
            {SKIN_TYPES.find((s) => s.value === skinType)!.emoji} {SKIN_TYPES.find((s) => s.value === skinType)!.label}
            {goal && ` · ${GOALS.find((g) => g.value === goal)!.emoji} ${GOALS.find((g) => g.value === goal)!.label}`}
          </p>
        </div>
        ...
        <div className="mt-3 space-y-1.5">
          {steps.map((s, i) => (
            <div key={i} className="flex items-center gap-2 rounded-lg bg-pink-50 px-3 py-2.5 dark:bg-pink-950">
        ...
        <button type="button" onClick={reset} className="mt-3 w-full rounded-xl border border-pink-200 py-2 text-xs font-bold text-pink-700 hover:bg-pink-50 dark:border-pink-800 dark:text-pink-300">
          بناء روتين جديد 🔄
        </button>
      </div>
    );
  ...
  return (
    <div className={cn('rounded-2xl border border-pink-100 bg-white p-5 dark:border-pink-900 dark:bg-gray-900', className)}>
      ...
      <div className="mt-3 flex gap-1">
        <div className={cn('h-1 flex-1 rounded-full', step === 'skin' ? 'bg-pink-500' : 'bg-pink-200 dark:bg-pink-800')} />
        <div className={cn('h-1 flex-1 rounded-full', step === 'goal' ? 'bg-pink-500' : 'bg-gray-200 dark:bg-gray-700')} />
      </div>

      {/* Options */}
      <div className="mt-3 space-y-1.5">
        {step === 'skin' && SKIN_TYPES.map((s) => (
          <button
            key={s.value}
            type="button"
            onClick={() => { setSkinType(s.value); setStep('goal'); }}
        ...
        {step === 'goal' && GOALS.map((g) => (
          <button
            key={g.value}
            type="button"
            onClick={() => { setGoal(g.value); setStep('result'); }}
            className="flex w-full items-center gap-2 rounded-xl bg-pink-50 px-3 py-2.5 text-xs font-bold text-pink-700 hover:bg-pink-100 dark:bg-pink-950 dark:text-pink-300 dark:hover:bg-pink-900 transition-colors"
          >
