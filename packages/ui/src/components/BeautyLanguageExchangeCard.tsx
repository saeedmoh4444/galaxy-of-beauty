...
export function BeautyLanguageExchangeCard({
  fromLang = 'ar',
  toLang: _toLang = 'en',
  className = '',
  title = 'قاموس الجمال',
    ...
    >
      <div className="flex items-center gap-2">
        ...
        <div>
          ...
          <p className="text-[10px] text-teal-500 dark:text-teal-400">
            {fromLang === 'ar' ? arToEnLabel : enToArLabel}
          </p>
        </div>
      </div>
      ...
      <div className="mt-3 grid grid-cols-2 gap-1.5">
        {TERMS.map((t) => (
          <div
            key={t.ar}
            className="flex items-center gap-2 rounded-lg bg-teal-50 px-2.5 py-2 dark:bg-teal-950"
          >
