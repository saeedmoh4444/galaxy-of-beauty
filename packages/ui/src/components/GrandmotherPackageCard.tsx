...
interface OccasionDef { emoji: string; title: string; look: string; price: number; }
...
export function GrandmotherPackageCard({ occasion, grandmaName, onBook, className = '' }: GrandmotherPackageCardProps): JSX.Element {
  ...
  return (
    <div className={cn('rounded-2xl border border-amber-100 bg-gradient-to-br from-amber-50 to-yellow-50 p-5 dark:border-amber-900 dark:from-amber-950 dark:to-yellow-950', className)}>
      <div className="text-center">
        ...
        <p className="text-[10px] text-amber-600 dark:text-amber-400">{grandmaName ? `${grandmaName} — ` : ''}{o.look}</p>
      </div>
      ...
      <div className="mt-3 rounded-xl bg-white/60 p-3 dark:bg-gray-800/60">
        ...
        <div className="mt-1 grid grid-cols-2 gap-1 text-[10px] text-amber-700 dark:text-amber-300">
          <span>• مكياج راقٍ</span><span>• تسريحة</span>
          <span>• مانيكير</span><span>• ضيافة</span>
          <span>• وقت إضافي</span><span>• صورة تذكارية</span>
        </div>
      </div>

      <div className="mt-3 flex items-center justify-between">
        <p className="text-lg font-bold text-amber-800 dark:text-amber-200">{o.price} ر.س</p>
        <button type="button" onClick={onBook} className="rounded-xl bg-amber-600 px-4 py-2.5 text-xs font-bold text-white hover:bg-amber-700 active:scale-[0.98] transition-all">احجزي 💛</button>
      </div>
    </div>
  );
}
