...
export function BeautyMoodTrackerCard({ onSelectMood, lastMood, className = '' }: BeautyMoodTrackerCardProps): JSX.Element {
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <div>
            ...
            <p className="text-[10px] text-violet-500 dark:text-violet-400">{lastMood ? `آخر مزاج: ${lastMood}` : 'تتبعي مزاجكِ بعد كل جلسة'}</p>
          </div>
        </div>
      </div>
      ...
      <div className="mt-3 flex justify-center gap-2">
        {MOODS.map((mood) => (
          <button
            key={mood.value}
            type="button"
            onClick={() => { setSelected(mood.value); onSelectMood?.(mood); }}
            className={cn(
          ...
          >
            ...
            <span className="text-[9px] font-medium text-text-secondary dark:text-gray-300">{mood.label}</span>
          </button>
        ))}
      </div>

      <p className="mt-2 text-center text-[9px] text-text-tertiary dark:text-gray-500">🎭 مزاجكِ جزء من رحلة جمالكِ</p>
    </div>
  );
}
