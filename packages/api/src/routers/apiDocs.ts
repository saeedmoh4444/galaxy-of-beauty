...
export const apiDocsRouter = router({
  ...
  openapi: publicProcedure.query(() => ({
    ...
    servers: [
      { url: 'https://galaxyofbeauty.sa/api/trpc', description: 'Production' },
      { url: 'http://localhost:3000/api/trpc', description: 'Local' },
    ],
    paths: API_REFERENCE.categories.reduce(
      (acc, cat) => {
        cat.routers.forEach((r) => {
          ...
          (acc as Record<string, unknown>)[key] = {
            get: {
              summary: `${r} — ${cat.name}`,
              tags: [cat.name],
              responses: { '200': { description: 'tRPC procedure response' } },
            },
          };
        });
        return acc;
