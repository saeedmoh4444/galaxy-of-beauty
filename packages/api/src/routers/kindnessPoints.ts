...
export const kindnessPointsRouter = router({
  ...
  earnTransaction: customerProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      const pointsMap: Record<string, number> = {
        answer_question: 10,
        give_advice: 25,
        mentor_new_member: 50,
        organize_event: 100,
        donate_beauty_bank: 200,
      };
      const points = pointsMap[input.action] || 5;

      let account = await prisma.kindnessAccount.findUnique({ where: { userId: ctx.user.id } });
      if (!account)
        account = await prisma.kindnessAccount.create({ data: { userId: ctx.user.id, points: 0 } });

      await prisma.kindnessTransaction.create({
        data: { accountId: account.id, points, reason: input.action },
      });
      const updated = await prisma.kindnessAccount.update({
        where: { id: account.id },
  ...
  leaderboard: customerProcedure
    ...
    .query(async ({ input }) => {
      return prisma.kindnessAccount.findMany({
        orderBy: { points: 'desc' },
        take: input.limit,
  ...
  history: customerProcedure
    ...
    .query(async ({ ctx, input }) => {
      ...
      return prisma.kindnessTransaction.findMany({
