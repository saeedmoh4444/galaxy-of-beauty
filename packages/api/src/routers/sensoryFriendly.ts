...
export const sensoryFriendlyRouter = router({
  listSalons: publicProcedure
    ...
    .query(async ({ input }) => {
      const [items, total] = await Promise.all([
        prisma.sensoryFriendlySalon.findMany({ where: { isCertified: true }, skip: (input.page - 1) * input.limit, take: input.limit }),
  ...
  register: customerProcedure
    .input(z.object({ salonName: z.string().min(2).max(200), features: z.array(z.string()), city: z.string().optional() }))
    .mutation(async ({ ctx, input }) => {
      return prisma.sensoryFriendlySalon.create({ data: { name: input.salonName, features: input.features, city: input.city ?? null, ownerId: ctx.user.id } });
    }),
});
