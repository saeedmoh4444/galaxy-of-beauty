  ...
  policy: customerProcedure.query(() => ({
    coverage: [
      ...
      },
      {
        ...
        descEn: 'Extra wallet credit for future services',
      },
    ],
    eligibility: [
      'جميع الخدمات مشمولة',
      'خلال ٤٨ ساعة من الخدمة',
      'الخدمات التي تقدمها فنيات معتمدات',
    ],
  })),
  ...
  claim: customerProcedure
    .input(
      z.object({
        ...
        compensationType: z.enum(['redo', 'refund', 'credit']),
      }),
    )
    .mutation(async ({ ctx, input }) => {
      ...
      const compensationMap: Record<string, number> = {
        redo: 0,
        refund: Number(booking.totalAmount || 0),
        credit: Math.round(Number(booking.totalAmount || 0) * WARRANTY_CREDIT_RATE),
      };
