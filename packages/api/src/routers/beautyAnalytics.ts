...
export const beautyAnalyticsRouter = router({
  ...
  byCategory: customerProcedure.query(async ({ ctx }) => {
    ...
    (bookings as any[]).forEach((b: any) => {
      ...
      categoryMap.set(catName, { count: prev.count + 1, spent: prev.spent + Number(b.totalAmount || 0) });
    });

    return Array.from(categoryMap.entries()).map(([category, data]) => ({
      category, count: data.count, spent: data.spent,
  ...
  monthlyTrend: customerProcedure.query(async ({ ctx }) => {
    ...
    (bookings as any[]).forEach((b: any) => {
      const month = new Date(b.createdAt).toLocaleDateString('ar-SA', { month: 'short', year: 'numeric' });
