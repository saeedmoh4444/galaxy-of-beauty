...
export const accountabilityRouter = router({
  myPartners: customerProcedure
    ...
    .query(async ({ ctx, input }) => {
      return prisma.accountabilityPartner.findMany({
  ...
  findPartner: customerProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      const match = await prisma.accountabilityPartner.findFirst({
        where: { goal: { contains: input.goal }, userId: { not: ctx.user.id } },
      });
      if (!match) {
        return prisma.accountabilityPartner.create({
          data: { userId: ctx.user.id, goal: input.goal, status: 'SEEKING' },
        });
      }
  ...
  stats: customerProcedure.query(async ({ ctx }) => {
    ...
    const best = await prisma.accountabilityPartner.findFirst({
      where: { userId: ctx.user.id },
      orderBy: { streak: 'desc' },
      select: { streak: true },
    });
