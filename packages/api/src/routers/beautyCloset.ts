...
export const beautyClosetRouter = router({
  myProducts: customerProcedure.query(async ({ ctx }) => {
    return prisma.beautyClosetProduct.findMany({
  ...
  addProduct: customerProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      return prisma.beautyClosetProduct.create({
        data: {
          userId: ctx.user.id,
          ...input,
          openDate: input.openDate ? new Date(input.openDate) : undefined,
        },
      });
    }),
  updateUsage: customerProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      return prisma.beautyClosetProduct.updateMany({
        where: { id: input.id, userId: ctx.user.id },
        data: { usagePct: input.usagePct },
      });
    }),
