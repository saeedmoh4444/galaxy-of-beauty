...
export const beautyCouponsRouter = router({
  ...
  validate: customerProcedure
    ...
    .query(async ({ input }) => {
      const coupon = await prisma.beautyCoupon.findFirst({
        where: { code: input.code.toUpperCase(), isActive: true, expiresAt: { gte: new Date() } },
      });
  ...
  create: adminProcedure
    .input(
      z.object({
        ...
        discountPercent: z.number().int().min(5).max(50),
        maxUses: z.number().int().positive().default(100),
        expiresAt: z.string(),
      }),
    )
    .mutation(async ({ input }) =>
      prisma.beautyCoupon.create({
        data: {
