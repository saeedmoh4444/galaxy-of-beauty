...
export const bookClubRouter = router({
  list: publicProcedure
    ...
    .query(async ({ input }) => {
      const [items, total] = await Promise.all([
        prisma.bookClub.findMany({ orderBy: { members: 'desc' }, skip: (input.page - 1) * input.limit, take: input.limit }),
  ...
  join: customerProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      const existing = await prisma.bookClubMember.findUnique({ where: { clubId_userId: { clubId: input.clubId, userId: ctx.user.id } } });
      if (!existing) {
        await prisma.bookClubMember.create({ data: { clubId: input.clubId, userId: ctx.user.id } });
        await prisma.bookClub.update({ where: { id: input.clubId }, data: { members: { increment: 1 } } });
      }
      return { success: true };
    }),
  ...
  myClubs: customerProcedure
    ...
    .query(async ({ ctx, input }) => prisma.bookClubMember.findMany({ where: { userId: ctx.user.id }, include: { club: true }, take: input.limit })),
});
