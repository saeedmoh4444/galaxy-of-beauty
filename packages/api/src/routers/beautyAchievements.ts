...
const ACHIEVEMENTS = [
  ...
  { key: 'streak_7', emoji: '🔥', title: '7 أيام متتالية', description: 'حافظتِ على روتينك 7 أيام' },
...
export const beautyAchievementsRouter = router({
  ...
  myAchievements: customerProcedure.query(async ({ ctx }) => {
    const earned = await prisma.customerAchievement.findMany({ where: { userId: ctx.user.id }, select: { achievementKey: true, earnedAt: true } });
  ...
  checkAndAward: customerProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      const existing = await prisma.customerAchievement.findUnique({ where: { userId_achievementKey: { userId: ctx.user.id, achievementKey: input.key } } });
      if (!existing && ACHIEVEMENTS.some((a) => a.key === input.key)) {
        await prisma.customerAchievement.create({ data: { userId: ctx.user.id, achievementKey: input.key } });
        return { awarded: true };
      }
  ...
  stats: customerProcedure.query(async ({ ctx }) => {
    ...
    return { earned, total: ACHIEVEMENTS.length, pct: Math.round((earned / ACHIEVEMENTS.length) * 100) };
  }),
});
