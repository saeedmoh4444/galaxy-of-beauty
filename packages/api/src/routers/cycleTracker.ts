...
export const cycleTrackerRouter = router({
  ...
  updateSettings: customerProcedure
    .input(z.object({ cycleLength: z.number().min(20).max(45).optional(), periodLength: z.number().min(2).max(10).optional(), lastPeriodStart: z.string().optional() }))
    .mutation(async ({ ctx, input }) => {
      return db.cycleSettings.upsert({
        where: { userId: ctx.user.id },
        update: { ...input, lastPeriodStart: input.lastPeriodStart ? new Date(input.lastPeriodStart) : undefined },
        create: { userId: ctx.user.id, ...input, lastPeriodStart: input.lastPeriodStart ? new Date(input.lastPeriodStart) : undefined },
      });
    }),
  ...
  today: customerProcedure.query(async ({ ctx }) => {
    ...
    const todayEntry = await db.cycleEntry.findFirst({ where: { userId: ctx.user.id, dayNumber: currentDay }, orderBy: { createdAt: 'desc' } });

    return { currentDay, cycleLength, phase, nextPeriodDate: nextPeriod, daysUntilNext, todayEntry, hasSettings: !!settings?.lastPeriodStart };
  }),
  ...
  logDay: customerProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      const existing = await db.cycleEntry.findFirst({ where: { userId: ctx.user.id, dayNumber: input.dayNumber }, orderBy: { createdAt: 'desc' } });
      if (existing) return db.cycleEntry.update({ where: { id: existing.id }, data: { phase: phase.key, ...input } });
