...
export const birthdayRewardRouter = router({
  myReward: customerProcedure.query(async ({ ctx }) => {
    ...
    const reward = await db.birthdayReward.findUnique({ where: { userId_year: { userId: ctx.user.id, year } } });
    return reward;
  }),
  claim: customerProcedure.mutation(async ({ ctx }) => {
    const year = new Date().getFullYear();
    const existing = await db.birthdayReward.findUnique({ where: { userId_year: { userId: ctx.user.id, year } } });
    ...
    await db.birthdayReward.update({ where: { id: existing.id }, data: { claimed: true, claimedAt: new Date(), promoCode: code } });
