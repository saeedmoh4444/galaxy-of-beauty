...
export const salonMembershipRouter = router({
  ...
  subscribe: customerProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      return prisma.salonMembership.upsert({
        where: { userId: ctx.user.id },
        create: { userId: ctx.user.id, tier: input.tier, autoRenew: input.autoRenew, expiresAt },
        update: { tier: input.tier, autoRenew: input.autoRenew, expiresAt, status: 'ACTIVE' },
      });
    }),
