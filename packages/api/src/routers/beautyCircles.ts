...
export const beautyCirclesRouter = router({
  ...
  getById: customerProcedure
    ...
    .query(async ({ input }) => {
      const circle = await prisma.beautyCircle.findUnique({
        where: { id: input.id },
        include: {
          circleMembers: {
  ...
  create: customerProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      return prisma.beautyCircle.create({
        data: {
          name: input.name,
          topic: input.topic,
  ...
  join: customerProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      const existing = await prisma.beautyCircleMember.findUnique({
        where: { circleId_userId: { circleId: input.circleId, userId: ctx.user.id } },
      });
      if (!existing) {
        await prisma.beautyCircleMember.create({
          data: { circleId: input.circleId, userId: ctx.user.id },
        });
        await prisma.beautyCircle.update({
  ...
  leave: customerProcedure
    .input(z.object({ circleId: z.number().int().positive() }))
    .mutation(async ({ ctx, input }) => {
      const member = await prisma.beautyCircleMember.findUnique({
        where: { circleId_userId: { circleId: input.circleId, userId: ctx.user.id } },
      });
  ...
  myCircles: customerProcedure.query(async ({ ctx }) => {
    return prisma.beautyCircleMember.findMany({
