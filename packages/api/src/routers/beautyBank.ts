...
export const beautyBankRouter = router({
  ...
  donate: customerProcedure
    .input(
      z.object({
        amount: z.number().int().positive().min(10),
        message: z.string().max(200).optional(),
      }),
    )
    .mutation(async ({ ctx, input }) => {
      return prisma.beautyBank.create({
        data: { donorId: ctx.user.id, amount: input.amount, message: input.message ?? null },
      });
    }),
  ...
  myDonations: customerProcedure
    ...
    .query(async ({ ctx, input }) => {
      return prisma.beautyBank.findMany({
        where: { donorId: ctx.user.id },
  ...
  leaderboard: publicProcedure
    .input(z.object({ limit: z.number().int().min(1).max(50).default(10) }))
    .query(async ({ input }) => {
      return prisma.beautyBank.groupBy({
        by: ['donorId'],
        _sum: { amount: true },
