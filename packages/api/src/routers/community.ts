...
export const communityRouter = router({
  ...
  feed: protectedProcedure
    ...
    .query(async ({ input }) => {
      ...
      const [posts, total] = await Promise.all([
        db.communityPost.findMany({ orderBy: { createdAt: 'desc' }, skip, take: input.limit, include: { user: { select: { id: true, name: true, avatarUrl: true } }, _count: { select: { comments: true } } } }),
  ...
  create: customerProcedure
    .input(z.object({ content: z.string().min(1).max(1000), imageUrl: z.string().optional(), category: z.string().optional() }))
    .mutation(async ({ ctx, input }) => {
      return db.communityPost.create({ data: { userId: ctx.user.id, ...input }, include: { user: { select: { name: true, avatarUrl: true } } } });
    }),
  ...
  comments: protectedProcedure
    .input(z.object({ postId: z.number(), page: z.number().default(1), limit: z.number().default(20) }))
    .query(async ({ input }) => {
      return db.communityComment.findMany({ where: { postId: input.postId }, orderBy: { createdAt: 'asc' }, take: input.limit, include: { user: { select: { name: true, avatarUrl: true } } } });
    }),

  addComment: customerProcedure
    .input(z.object({ postId: z.number(), content: z.string().min(1).max(500) }))
    .mutation(async ({ ctx, input }) => {
      return db.communityComment.create({ data: { postId: input.postId, userId: ctx.user.id, content: input.content }, include: { user: { select: { name: true, avatarUrl: true } } } });
    }),
