...
export const timeCapsuleRouter = router({
  ...
  myCapsules: customerProcedure
    ...
    .query(async ({ ctx, input }) => {
      return prisma.timeCapsule.findMany({ where: { userId: ctx.user.id }, orderBy: { createdAt: 'desc' }, take: input.limit });
    }),

  open: customerProcedure
    .input(z.object({ capsuleId: z.number().int().positive() }))
    .mutation(async ({ ctx, input }) => {
      const capsule = await prisma.timeCapsule.findFirst({ where: { id: input.capsuleId, userId: ctx.user.id } });
  ...
  upcoming: customerProcedure
    ...
    .query(async ({ ctx, input }) => {
      return prisma.timeCapsule.findMany({ where: { userId: ctx.user.id, opened: false, openDate: { gte: new Date().toISOString().slice(0, 10) } }, orderBy: { openDate: 'asc' }, take: input.limit });
    }),
});
