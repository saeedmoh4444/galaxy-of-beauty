...
export const dailyBeautyTipRouter = router({
  ...
  list: adminProcedure
    ...
    .query(async ({ input }) => {
      const [items, total] = await Promise.all([
        prisma.dailyBeautyTip.findMany({ orderBy: { createdAt: 'desc' }, skip: (input.page - 1) * input.limit, take: input.limit }),
  ...
  create: adminProcedure
    .input(z.object({ emoji: z.string().default('💡'), tip: z.string().min(5).max(300), category: z.string().default('عناية') }))
    .mutation(async ({ input }) => {
      return prisma.dailyBeautyTip.create({ data: { emoji: input.emoji, tip: input.tip, category: input.category } });
    }),
});
