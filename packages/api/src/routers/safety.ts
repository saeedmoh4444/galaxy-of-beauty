...
export const safetyRouter = router({
  ...
  getEscortStatus: customerProcedure
    ...
    .query(async ({ ctx, input }) => {
      return prisma.escortRequest.findFirst({
        where: { bookingId: input.bookingId, userId: ctx.user.id },
  ...
  startSharing: customerProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      const session = await prisma.locationShare.create({
        data: { userId: ctx.user.id, bookingId: input.bookingId, active: true },
      });
  ...
  stopSharing: customerProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      const session = await prisma.locationShare.findFirst({
  ...
  checkIn: customerProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      return prisma.safetyCheckIn.create({
        data: { userId: ctx.user.id, bookingId: input.bookingId, type: 'HOME_SAFE' },
      });
    }),
