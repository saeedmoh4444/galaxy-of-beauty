...
const ACTIVE_DEALS = [
  ...
  {
    id: 2,
    titleAr: 'خصم ٣٠٪ على العناية بالبشرة',
    category: 'skincare',
    discount: 30,
    endsIn: '٤ ساعات',
    emoji: '',
  ...
  create: customerProcedure
    .input(
      z.object({
        ...
        maxDiscount: z.number().min(10).max(80).default(30),
      }),
    )
    .mutation(async ({ ctx, input }) =>
      prisma.saleAlert.create({
        data: { userId: ctx.user.id, categories: input.categories, maxDiscount: input.maxDiscount },
      }),
    ),
