...
export const giftCardMarketRouter = router({
  listings: customerProcedure.query(() =>
    prisma.giftCardListing.findMany({ orderBy: { createdAt: 'desc' }, take: 50 }),
  ),
  myListings: customerProcedure.query(async ({ ctx }) =>
    prisma.giftCardListing.findMany({ where: { sellerId: ctx.user.id }, take: 20 }),
  ),
    ...
    .mutation(async ({ ctx, input }) =>
      prisma.giftCardListing.create({
        data: {
          ...
          discount: Math.round(((input.value - input.sellingPrice) / input.value) * 100),
        },
      }),
    ),
  buy: customerProcedure.input(z.object({ listingId: z.number() })).mutation(async ({ input }) => {
