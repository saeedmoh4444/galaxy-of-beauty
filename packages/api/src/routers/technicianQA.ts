  ...
  list: publicProcedure
    ...
    .query(async ({ input }) => {
      ...
      const [items, total] = await Promise.all([
        prisma.qAQuestion.findMany({ where, orderBy: { createdAt: 'desc' }, skip: (input.page - 1) * input.limit, take: input.limit }),
  ...
  ask: customerProcedure
    ...
    .mutation(async ({ ctx, input }) =>
      prisma.qAQuestion.create({
        data: { userId: ctx.user.id, userName: ctx.user.email, question: input.question, category: input.category },
      })),
