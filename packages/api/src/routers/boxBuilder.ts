...
const PRODUCT_CATALOG = [
  ...
  {
    id: 7,
    nameAr: 'شامبو خالي من الكبريتات',
  ...
  {
    id: 8,
    nameAr: 'مجموعة العناية بالأظافر',
...
export const boxBuilderRouter = router({
  ...
  build: customerProcedure
    .input(
      z.object({
        name: z.string().min(1),
        productIds: z.array(z.number()).min(3).max(6),
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      const discount =
        input.frequency === 'monthly'
          ? Math.round(subtotal * BOX_MONTHLY_DISCOUNT)
          : Math.round(subtotal * BOX_REGULAR_DISCOUNT);
      const box = await prisma.beautyBox.create({
        data: {
          userId: ctx.user.id,
          name: input.name,
          products: selected,
