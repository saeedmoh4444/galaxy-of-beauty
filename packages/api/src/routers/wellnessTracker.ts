...
export const wellnessTrackerRouter = router({
  today: customerProcedure.query(async ({ ctx }) => {
    ...
    return prisma.wellnessCheckin.findUnique({ where: { userId_date: { userId: ctx.user.id, date: today } } });
  }),

  checkin: customerProcedure
    .input(z.object({ water: z.number().optional(), sleep: z.number().optional(), mood: z.number().optional(), steps: z.number().optional(), skincare: z.boolean().optional(), notes: z.string().optional() }))
  ...
  weekly: customerProcedure.query(async ({ ctx }) => {
    ...
    for (let i = 6; i >= 0; i--) {
      ...
      const entry = await prisma.wellnessCheckin.findUnique({ where: { userId_date: { userId: ctx.user.id, date: dateStr } } });
