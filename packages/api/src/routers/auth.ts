...
export const authRouter = router({
  ...
  // ──────────────────────────────────────────────────────
  logout: protectedMutation.input(z.object({})).mutation(async ({ ctx }) => {
    try {
      await prisma.refreshToken.updateMany({
        where: {
          userId: ctx.user.id,
  ...
  // ──────────────────────────────────────────────────────
  updateProfile: protectedMutation.input(updateProfileSchema).mutation(async ({ ctx, input }) => {
    try {
  ...
  // Change password
  // ──────────────────────────────────────────────────────
  changePassword: protectedMutation.input(changePasswordSchema).mutation(async ({ ctx, input }) => {
    try {
      const user = await prisma.user.findUnique({
        where: { id: ctx.user.id },
      });
  ...
  // ──────────────────────────────────────────────────────
  forgotPassword: publicMutation.input(forgotPasswordSchema).mutation(async ({ input }) => {
    try {
  ...
  // ──────────────────────────────────────────────────────
  resetPassword: publicMutation.input(resetPasswordSchema).mutation(async ({ input }) => {
    try {
  ...
  // ──────────────────────────────────────────────────────
  verifyEmail: publicMutation.input(verifyEmailSchema).mutation(async ({ input }) => {
    try {
  ...
  // ──────────────────────────────────────────────────────
  resendVerification: protectedMutation.input(z.object({})).mutation(async ({ ctx }) => {
    try {
      const token = generateToken();
  ...
  // ──────────────────────────────────────────────────────
  setup2FA: protectedMutation.input(z.object({})).mutation(async ({ ctx }) => {
    try {
      const user = await prisma.user.findUnique({
        where: { id: ctx.user.id },
      });
  ...
  // ──────────────────────────────────────────────────────
  verify2FA: protectedMutation.input(twoFactorVerifySchema).mutation(async ({ ctx, input }) => {
    try {
      const user = await prisma.user.findUnique({
        where: { id: ctx.user.id },
      });
  ...
  // Disable 2FA
  // ──────────────────────────────────────────────────────
  disable2FA: protectedMutation.input(z.object({})).mutation(async ({ ctx }) => {
    try {
      await prisma.user.update({
        where: { id: ctx.user.id },
        data: {
