...
export const customerJourneyRouter = router({
  myStage: customerProcedure.query(async ({ ctx }) => {
    ...
    return {
      stage,
      bookings,
      nextStage: STAGES[Math.min(STAGES.indexOf(stage) + 1, STAGES.length - 1)],
    };
  }),
  ...
  milestones: customerProcedure
    ...
    .query(async ({ ctx, input }) =>
      prisma.customerMilestone.findMany({
  ...
  recordMilestone: customerProcedure
    ...
    .mutation(async ({ ctx, input }) =>
      prisma.customerMilestone.create({
        data: { userId: ctx.user.id, type: input.type, description: input.description ?? null },
      }),
    ),
});
