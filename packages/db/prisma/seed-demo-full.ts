...
async function main() {
  ...
  const cityIdMap = new Map(cityRecords.map((c) => [c.nameAr, c.id]));

  let areasSeeded = 0;
  for (const [cityName, areaNames] of Object.entries(areaMap)) {
    const cityId = cityIdMap.get(cityName);
    if (cityId) {
      for (const areaName of areaNames) {
        await prisma.area.upsert({
          where: { cityId_nameAr: { cityId, nameAr: areaName } },
          update: {},
          create: { cityId, nameAr: areaName, nameEn: areaName, isActive: true },
        });
        areasSeeded++;
      }
    }
  }
  console.log(`  ${areasSeeded} areas seeded across ${Object.keys(areaMap).length} cities`);
