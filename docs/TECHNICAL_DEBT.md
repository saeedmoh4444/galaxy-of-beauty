...
| P2-05 | 206 ESLint-disable directives to review               | ✅ Reviewed 2026-08-17: 98 remain repo-wide (was ~226) — web 51, api 19+9=28ish, mobile 8. Every remaining disable carries a `-- reason` (ARCH-007). The sweep fixed ~90 web + ~41 api `any`s. 9 API files keep reasoned `prisma as any` — they query fields that don't exist in the schema (latent runtime bugs, documented per file)                                                                                                                                                                                                                                      |
