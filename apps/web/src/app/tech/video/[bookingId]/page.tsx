...
// A4 — technician-side video session entry. Mirrors the customer session
// page (/video/[bookingId]); the API accepts either booking participant.

export default function TechVideoSessionPage(): JSX.Element {
  const { t } = useLocale();
  ...
  const handleStart = async () => {
    ...
    try {
      ...
      router.push(`/tech/video/${bookingId}/room?room=${roomId}`);
    } catch {
      setJoining(false);
    }
  };
        ...
        {isLoading ? (
          ...
          >
            ...
            <div className="mt-4">
              <Button
                onClick={() =>
                  router.push(`/tech/video/${bookingId}/room?room=${sess.roomId as string}`)
                }
              >
