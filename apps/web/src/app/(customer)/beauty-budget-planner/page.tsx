  ...
  return (
    <DashboardLayout role="CUSTOMER">
      <div className="mx-auto max-w-3xl space-y-6">
        <div><h1 className="text-2xl font-bold">📊 مخطط الميزانية</h1><p className="mt-1 text-sm text-text-secondary">خططي لمصاريف جمالكِ السنوية</p></div>
        <div className="grid gap-4 sm:grid-cols-3"><Card padding="lg" className="text-center bg-amber-50"><p className="text-2xl font-extrabold">{formatCurrency(totalBudget)}</p><p className="text-xs text-text-secondary">الميزانية</p></Card><Card padding="lg" className="text-center bg-green-50"><p className="text-2xl font-extrabold text-green-600">{formatCurrency(allocated)}</p><p className="text-xs text-text-secondary">مخصص</p></Card><Card padding="lg" className="text-center bg-blue-50"><p className="text-2xl font-extrabold text-blue-600">{formatCurrency(remaining)}</p><p className="text-xs text-text-secondary">متبقي</p></Card></div>
        {isLoading ? <CardSkeleton/> : <div className="space-y-3">{CATEGORIES.map(c=>{const spent = (items??[]).find((i:Record<string,unknown>)=>i.category===c.key)?.spent as number??0;const pct=Math.min(100,Math.round((spent/c.budget)*100));return(<Card key={c.key} padding="md"><div className="flex items-center gap-4"><span className="text-3xl">{c.emoji}</span><div className="flex-1"><p className="font-bold">{c.name}</p><p className="text-xs text-text-secondary">الميزانية: {formatCurrency(c.budget)}</p><div className="mt-2 h-2 bg-surface-muted rounded-full"><div className="h-2 rounded-full" style={{width:`${pct}%`,backgroundColor:spent>c.budget?'#ef4444':c.color}}/></div></div><p className={`font-bold ${spent>c.budget?'text-red-600':'text-brand-600'}`}>{formatCurrency(spent)}</p></div></Card>);})}</div>}
      </div>
    </DashboardLayout>
  );
}
