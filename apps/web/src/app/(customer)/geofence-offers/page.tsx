  ...
  return (
    <DashboardLayout role="CUSTOMER">
      <div className="mx-auto max-w-2xl space-y-6">
        <div><h1 className="text-2xl font-bold">📍 عروض بالقرب منك</h1><p className="mt-1 text-sm text-gray-500">عروض حصرية من الصالونات القريبة من موقعك</p></div>
        ...
        {isLoading ? <div className="space-y-3">{Array.from({length:3},(_,i)=><CardSkeleton key={i}/>)}</div> :
          <div className="space-y-3">{items.map((o: Record<string,unknown>) => (
            <Card key={o.id as number} padding="lg" className="flex items-center gap-4 hover:shadow-md transition-all">
              ...
              <div className="flex-1"><h3 className="font-bold">{o.titleAr as string}</h3><p className="text-xs text-gray-500">{o.salonName as string} · {o.distance as string} · {o.city as string}</p></div>
