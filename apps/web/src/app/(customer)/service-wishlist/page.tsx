...
import { Card, CardSkeleton, EmptyState, Button, Modal, formatCurrency } from '@galaxy/shared';
import { DashboardLayout } from '@/components/layout/DashboardLayout';

export default function ServiceWishlistPage(): JSX.Element {
  const { data: items, refetch } = api.serviceWishlist.myWishlist.useQuery() as { data: Array<Record<string,unknown>> | undefined; refetch: () => void };
  const addMut = api.serviceWishlist.add.useMutation({ onSuccess: () => { setShow(false); refetch(); } });
  const removeMut = api.serviceWishlist.remove.useMutation({ onSuccess: () => refetch() });

  const [show, setShow] = useState(false); const [name, setName] = useState(''); const [price, setPrice] = useState(100);
  const list = items ?? [];
          ...
          <div className="space-y-3">{list.map((i: Record<string,unknown>) => (
            <Card key={i.id as number} padding="md" className="flex items-center justify-between">
              ...
              <div className="text-right">
                ...
                <button onClick={() => removeMut.mutate({ id: i.id as number })} className="block mt-1 text-xs text-red-400">🗑️</button>
              </div>
            </Card>
          ))}</div>
        }
        <Modal open={show} onClose={() => setShow(false)} title="إضافة خدمة"><div className="space-y-3">
          <input value={name} onChange={(e) => setName(e.target.value)} placeholder="اسم الخدمة" className="w-full rounded-lg border px-3 py-2 text-sm dark:border-gray-700 dark:bg-gray-800" />
          <input type="number" value={price} onChange={(e) => setPrice(parseInt(e.target.value) || 0)} placeholder="السعر الحالي" className="w-full rounded-lg border px-3 py-2 text-sm dark:border-gray-700 dark:bg-gray-800" />
          <Button onClick={() => { if (name.trim() && price > 0) addMut.mutate({ serviceName: name.trim(), currentPrice: price }); }} loading={addMut.isPending} className="w-full">📝 إضافة</Button>
        </div></Modal>
      </div>
    </DashboardLayout>
  );
