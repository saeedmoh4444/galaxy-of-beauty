  ...
  return (
    <DashboardLayout role="CUSTOMER">
      <div className="mx-auto max-w-3xl space-y-6">
        <div><h1 className="text-2xl font-bold">🧾 الفواتير</h1><p className="mt-1 text-sm text-text-secondary">سجل مدفوعاتكِ وفواتيركِ</p></div>

        <div className="grid gap-4 sm:grid-cols-2">
          <Card padding="lg" className="text-center"><p className="text-2xl font-extrabold">{completed.length}</p><p className="text-xs text-text-secondary">فواتير مدفوعة</p></Card>
          <Card padding="lg" className="text-center"><p className="text-2xl font-extrabold text-green-600">{formatCurrency(totalSpent)}</p><p className="text-xs text-text-secondary">إجمالي المدفوعات</p></Card>
        </div>

        {isLoading ? <div className="space-y-3">{Array.from({length:3},(_,i)=><CardSkeleton key={i}/>)}</div> :
          completed.length === 0 ? <Card padding="lg" className="text-center py-8"><p className="text-4xl mb-2">🧾</p><p className="text-text-secondary">لا توجد فواتير بعد</p></Card> :
              ...
              <Card key={b.id as number} padding="md">
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <p className="text-xs text-text-secondary">{new Date((slot?.startAt ?? b.createdAt) as string).toLocaleDateString('ar-SA', {day:'numeric', month:'long', year:'numeric'})} · {b.bookingCode as string}</p>
                  </div>
