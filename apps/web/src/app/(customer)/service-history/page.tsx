...
export default function ServiceHistoryPage(): JSX.Element {
  ...
  const serviceCounts: Record<number, { count: number; lastDate: string; title: string; price: number }> = {};
  bookings.filter((b: any) => b.status === 'COMPLETED').forEach((b: any) => {
    const sid = b.serviceId;
    if (!serviceCounts[sid]) {
      serviceCounts[sid] = { count: 0, lastDate: b.createdAt, title: ar((b.service as any)?.titleJson) || `خدمة #${sid}`, price: Number(b.totalAmount) };
    }
  ...
  const recentBookings = [...bookings].sort((a: any, b: any) => new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime());
    ...
    <DashboardLayout role="CUSTOMER">
      <div className="mx-auto max-w-3xl space-y-8">
        ...
        <div>
          ...
          {isLoading ? <CardSkeleton /> : isError ? <ErrorAlert message="فشل التحميل" onRetry={() => refetch()} /> : recentBookings.length === 0 ? <EmptyState title="لا توجد حجوزات سابقة" /> : (
