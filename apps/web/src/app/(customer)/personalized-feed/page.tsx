  ...
  return (
    <DashboardLayout role="CUSTOMER">
      <div className="mx-auto max-w-2xl space-y-6">
        <div><h1 className="text-2xl font-bold">🎯 خلاصتي</h1><p className="mt-1 text-sm text-gray-500">محتوى مخصص لكِ بناءً على اهتماماتكِ</p></div>
        {isLoading ? <div className="space-y-3">{Array.from({length:6},(_,i)=><CardSkeleton key={i}/>)}</div> :
          <div className="space-y-3">{items.map((item: Record<string,unknown>) => (
            <Card key={item.id as number} padding="md" className="flex items-center gap-4 hover:shadow-md transition-all">
              ...
              <div className="flex-1"><p className="font-bold text-sm">{item.title as string}</p><p className="text-xs text-gray-500">{item.technician ? `👩‍🎨 ${item.technician}` : item.brand ? `🏷️ ${item.brand}` : `💰 ${item.price as number} ر.س`}</p></div>
