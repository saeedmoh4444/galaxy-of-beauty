  ...
  return (
    <DashboardLayout role="CUSTOMER">
      <div className="mx-auto max-w-3xl space-y-6">
        <div><h1 className="text-2xl font-bold">📋 QR قائمة الخدمات</h1><p className="mt-1 text-sm text-text-secondary">ولدي كود QR لقائمة خدمات الفنيات لمشاركته مع العميلات</p></div>
          ...
          <Card padding="lg">
            <div className="space-y-3">{list.map((t: Record<string,unknown>) => (
              <div key={t.id as number} className="flex items-center justify-between rounded-xl bg-surface-muted dark:bg-gray-800 p-4">
                <div><p className="font-bold">{t.name as string}</p><p className="text-xs text-text-secondary">{t.services as string}</p></div>
