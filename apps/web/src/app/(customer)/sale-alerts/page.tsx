...
export default function SaleAlertsPage(): JSX.Element {
  ...
  const {
    ...
    isError,
    refetch,
  } = api.saleAlerts.myAlerts.useQuery() as {
    data: Array<Record<string, unknown>> | undefined;
    isLoading: boolean;
    isError: boolean;
    refetch: () => void;
  };
  const { data: deals } = api.saleAlerts.activeDeals.useQuery() as {
    data: Array<Record<string, unknown>> | undefined;
  };
  const createMut = api.saleAlerts.create.useMutation({
    onSuccess: () => {
      setShowAdd(false);
      setSelectedCats([]);
      refetch();
            ...
            <Button
              onClick={() => {
                if (selectedCats.length > 0)
                  createMut.mutate({ categories: selectedCats, maxDiscount });
              }}
