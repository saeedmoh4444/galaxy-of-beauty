...
export default function BeautyAdvisorPage(): JSX.Element {
  const [messages, setMessages] = useState<Array<{role:string;content:string}>>([{role:'assistant',content:'👋 مرحباً! أنا ليلى، مستشارة جمالكِ الشخصية. اسأليني أي سؤال عن العناية بالبشرة، المكياج، الشعر، أو أي نصيحة تجميلية!'}]);
  const [input, setInput] = useState('');
  const [question, setQuestion] = useState('');
  const chatEndRef = useRef<HTMLDivElement>(null);
  const { data: aiResponse, isFetching } = api.aiAssistant.ask.useQuery({ question }, { enabled: !!question }) as { data: Record<string,unknown> | undefined; isFetching: boolean };

  useEffect(() => { chatEndRef.current?.scrollIntoView({ behavior: 'smooth' }); }, [messages]);
  useEffect(() => {
    if (aiResponse && question) {
      const reply = (aiResponse.answer as string) ?? (aiResponse as any).message ?? 'عذراً، لم أستطع الإجابة.';
  ...
  return (
    <DashboardLayout role="CUSTOMER">
      <div className="mx-auto max-w-2xl flex flex-col" style={{height:'calc(100vh - 120px)'}}>
        ...
        <div className="flex-1 overflow-y-auto bg-white rounded-2xl border p-4 mb-3" style={{minHeight:'300px'}}>
        ...
        <div className="flex gap-2">
          ...
          <Button onClick={() => handleSend()} loading={isFetching} className="rounded-xl px-6">📤</Button>
        </div>
      </div>
    </DashboardLayout>
  );
