  ...
  return (
    <DashboardLayout role="CUSTOMER">
      <div className="mx-auto max-w-3xl space-y-6">
        <div><h1 className="text-2xl font-bold">📡 الأجهزة الذكية</h1><p className="mt-1 text-sm text-gray-500">اربطي أجهزة العناية الذكية لمتابعة بشرتكِ</p></div>
        <div className="grid gap-4 sm:grid-cols-3">{list.map((d: Record<string,unknown>) => (
          <Card key={d.key as string} padding="lg" className="text-center">
            ...
            <p className={`text-xs mt-1 ${d.status === 'connected' ? 'text-green-600' : 'text-gray-400'}`}>{d.status === 'connected' ? '🟢 متصل' : '⚫ غير متصل'}</p>
            <div className="mt-2">{(d.features as string[])?.slice(0,2).map((f: string) => <p key={f} className="text-[10px] text-gray-500">• {f}</p>)}</div>
