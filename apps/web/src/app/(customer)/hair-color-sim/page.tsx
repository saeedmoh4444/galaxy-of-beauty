            ...
            <div className="flex flex-wrap gap-3">
              {allColors.map((c: Record<string, unknown>) => (
                <button
                  key={c.id as string}
                  onClick={() => {
                    setSelected(c.id as string);
                    saveMut.mutate({ colorId: c.id as string });
                  }}
                  className={`flex flex-col items-center gap-1 rounded-xl p-3 transition-all ${selected === c.id ? 'ring-2 ring-brand-500 scale-105 shadow-lg' : 'hover:scale-105'}`}
                >
                  <div
