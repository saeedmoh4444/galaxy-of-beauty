  ...
  return (
    <DashboardLayout role="CUSTOMER">
      <div className="mx-auto max-w-2xl space-y-6">
        <div><h1 className="text-2xl font-bold">📦 توصيل سريع</h1><p className="mt-1 text-sm text-gray-500">منتجات تجميل توصل لباب بيتكِ</p></div>
        {result ? (
          <Card padding="lg" className="text-center border-2 border-green-300"><span className="text-6xl">✅</span><h2 className="mt-4 text-xl font-bold">تم الطلب!</h2><p className="font-bold mt-1">{result.product as string}</p><p className="text-sm text-gray-500">📦 {result.estimatedDelivery as string} · {formatCurrency(result.total as number)} ر.س</p></Card>
        ) : (
          <div className="space-y-3">{prods.map((p: Record<string,unknown>) => (
            <Card key={p.id as number} padding="md" className="flex items-center justify-between">
              <div className="flex items-center gap-3"><span className="text-3xl">{p.emoji as string}</span><div><p className="font-bold">{p.nameAr as string}</p><p className="text-xs text-gray-500">⏱️ {p.deliveryTime as string}</p></div></div>
