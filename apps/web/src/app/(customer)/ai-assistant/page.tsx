...
export default function AIAssistantPage(): JSX.Element {
  ...
  const { data: topics } = api.aiAssistant.topics.useQuery() as { data: Array<Record<string,unknown>> | undefined };
  const { data: answer, isLoading } = api.aiAssistant.ask.useQuery({ question: searchQ }, { enabled: searchQ.length > 1 }) as { data: Record<string,unknown> | undefined; isLoading: boolean };
  ...
  return (
    <DashboardLayout role="CUSTOMER">
      <div className="mx-auto max-w-2xl space-y-6">
        ...
        {isLoading ? <CardSkeleton /> : (answer as Record<string,unknown>) ? <Card padding="lg" className="border-2 border-brand-200 dark:border-brand-800 bg-gradient-to-br from-brand-50 to-purple-50 dark:from-brand-950 dark:to-purple-950"><div className="flex items-start gap-3"><span className="text-3xl">🤖</span><div><p className="font-bold text-sm mb-2">{(answer as Record<string,unknown>).question as string}</p><p className="text-sm leading-relaxed">{(answer as Record<string,unknown>).answer as string}</p></div></div></Card> : null}
      </div>
    </DashboardLayout>
  );
}
