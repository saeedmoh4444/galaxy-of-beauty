    ...
    <DashboardLayout userRole="CUSTOMER">
      <PageContainer width="default">
        ...
        <div className="rounded-2xl border border-edge-muted bg-surface-elevated p-6">
          ...
          <h4 className="mt-6 text-sm font-bold text-text-primary">{t('color.palette')}</h4>
          <div className="mt-3 flex gap-2">
            {s.colors.map((c, i) => (
              <div
                key={i}
                className="h-10 w-10 rounded-full border border-edge shadow-sm"
                style={{ backgroundColor: c }}
              />
            ))}
          </div>

          <h4 className="mt-6 text-sm font-bold text-text-primary">{t('color.suitableMakeup')}</h4>
          <div className="mt-2 space-y-2">
            {s.makeup.map((m, i) => (
              ...
              >
                <span>💄</span>
                <span className="text-sm text-text-secondary">{t(m)}</span>
              </div>
            ))}
          </div>

          <div className="mt-4 flex items-center justify-between border-t border-edge-muted pt-4">
