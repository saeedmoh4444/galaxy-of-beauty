...
export default function NewsletterPage(): JSX.Element {
  const { data: issues, isLoading } = api.newsletter.issues.useQuery() as { data: Array<Record<string,unknown>> | undefined; isLoading: boolean };
        ...
        {subscribed ? (
          ...
          <Card padding="lg">
            <div className="flex gap-2">
              <input type="email" value={email} onChange={(e) => setEmail(e.target.value)} placeholder="بريدكِ الإلكتروني" className="flex-1 rounded-lg border px-3 py-2 text-sm dark:border-gray-700 dark:bg-gray-800" />
              <Button onClick={() => { if (email.includes('@')) subscribeMut.mutate({ email }, { onSuccess: () => setSubscribed(true) }); }} loading={subscribeMut.isPending}>اشتراك</Button>
            </div>
          </Card>
        )}
