          ...
          allRoutines.map((r: Record<string, unknown>) => (
            <Card key={r.id as string} padding="lg">
              ...
              <div className="space-y-1">
                {(r.steps as Array<Record<string, unknown>>).map(
                  (s: Record<string, unknown>, i: number) => {
                    ...
                    return (
                      <div
                        key={i}
                        className={`flex items-center gap-3 rounded-lg p-3 transition-all ${done ? 'bg-green-50 dark:bg-green-950' : 'bg-surface-muted dark:bg-gray-800'}`}
                      >
                        <span className="text-2xl">{s.emoji as string}</span>
                        <span className="text-xs text-text-tertiary w-14">{s.time as string}</span>
                        <span
                          className={`flex-1 font-medium text-sm ${done ? 'line-through text-text-tertiary' : ''}`}
                        >
                          {s.task as string}
                        </span>
                        <input
