...
export default function SelfCarePage(): JSX.Element {
  ...
  const checkinMut = api.selfCare.checkin.useMutation({ onSuccess: () => { addToast('success', 'تم تسجيل تقييمكِ اليومي 🌸'); setNotes(''); } });
        ...
        {today ? (
          ...
          <Card padding="lg">
            ...
            <div className="flex justify-center gap-3">
              {MOODS.map((m, i) => (
                <button key={i} onClick={() => setMood(i + 1)} className={`text-3xl transition-all ${mood === i + 1 ? 'scale-125' : 'opacity-40 hover:opacity-70'}`}>{m}</button>
              ))}
            </div>
            ...
            <div className="mt-4 space-y-3">
              ...
              <input placeholder="ملاحظات (اختياري)" value={notes} onChange={(e) => setNotes(e.target.value)} className="w-full rounded-lg border border-gray-300 p-2 text-sm dark:border-gray-600 dark:bg-gray-800" />
            </div>
            <Button onClick={() => checkinMut.mutate({ mood, energy: energy || undefined, sleepHours: sleep ? Number(sleep) : undefined, waterGlasses: water ? Number(water) : undefined, notes: notes || undefined })} loading={checkinMut.isPending} className="mt-4 w-full">🌸 حفظ التقييم</Button>
          </Card>
        )}
