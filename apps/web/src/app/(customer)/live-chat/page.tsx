...
export default function LiveChatPage(): JSX.Element {
  ...
  const { data: history, refetch } = api.liveChat.history.useQuery() as { data: Array<Record<string,unknown>> | undefined; refetch: () => void };
  const sendMut = api.liveChat.send.useMutation({ onSuccess: () => { setMsg(''); refetch(); } });
      ...
      <div className="mx-auto max-w-2xl space-y-4">
        ...
        <Card padding="md" className="h-[60vh] flex flex-col">
          ...
          <div className="flex gap-2 mt-3 pt-3 border-t dark:border-gray-700">
            <input value={msg} onChange={(e) => setMsg(e.target.value)} onKeyDown={(e) => e.key === 'Enter' && msg.trim() && sendMut.mutate({ message: msg.trim() })} placeholder="اكتبي رسالتك..." className="flex-1 rounded-lg border px-3 py-2 text-sm dark:border-gray-700 dark:bg-gray-800" />
            <Button onClick={() => msg.trim() && sendMut.mutate({ message: msg.trim() })} loading={sendMut.isPending}>إرسال</Button>
          </div>
        </Card>
      </div>
    </DashboardLayout>
