  ...
  return (
    <DashboardLayout role="CUSTOMER">
      <div className="mx-auto max-w-3xl space-y-6">
        <div><h1 className="text-2xl font-bold">🚨 حجز طارئ</h1><p className="mt-1 text-sm text-text-secondary">حجز فوري خلال ٣ ساعات مع رسوم إضافية</p></div>
        ...
        <Card padding="lg"><h3 className="font-bold mb-3">💅 اختاري الخدمة</h3>
          <div className="flex flex-wrap gap-2 mb-4">{POPULAR_SERVICES.map(s => (
            <button key={s.id} onClick={()=>{setServiceId(s.id);setSelectedTech(null);setBookingCode(null);}} className={`rounded-full px-4 py-2 text-sm transition-all ${serviceId===s.id?'bg-red-600 text-white':'bg-surface-muted'}`}>{s.emoji} {s.name}</button>
          ))}</div>

          {isLoading ? <CardSkeleton/> :
            available.length === 0 ? <p className="text-sm text-text-tertiary text-center py-4">😔 لا توجد فنيات متاحات حالياً لهذه الخدمة</p> :
            <div className="space-y-2">{available.map((t: Record<string,unknown>) => (
              <button key={t.technicianId as number} onClick={()=>setSelectedTech(t.technicianId as number)} className={`w-full rounded-xl border-2 p-3 text-right transition-all ${selectedTech===t.technicianId?'border-red-400 bg-red-50':'border-gray-200'}`}>
                ...
                <div className="flex justify-between text-xs text-text-secondary mt-1"><span>⭐ {t.rating as number} · {t.city as string}</span><span>🕐 {t.nextSlot ? new Date(t.nextSlot as string).toLocaleTimeString('ar-SA',{hour:'2-digit',minute:'2-digit'}) : '—'}</span></div>
              </button>
            ))}</div>
          }
        </Card>

        {bookingCode ? <Card padding="lg" className="text-center border-2 border-green-300 bg-green-50"><p className="text-3xl">✅</p><p className="font-bold text-green-700 mt-2">تم الحجز الطارئ</p><p className="text-sm text-text-secondary">رمز الحجز: <span className="font-mono font-bold">{bookingCode}</span></p><p className="text-lg font-bold text-green-600 mt-1">الإجمالي: {formatCurrency(avail?.totalEstimate as number ?? 0)}</p></Card> :
