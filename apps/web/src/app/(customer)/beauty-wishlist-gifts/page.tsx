    ...
    <DashboardLayout role="CUSTOMER">
      <div className="mx-auto max-w-3xl space-y-6">
        ...
        {showForm && <Card padding="lg">
          <input value={title} onChange={e => setTitle(e.target.value)} placeholder="اسم السجل (مثال: هدايا عيد ميلادي)" className="w-full rounded-lg border px-3 py-2 text-sm mb-3 dark:border-gray-700 dark:bg-gray-800" />
          <select value={occasion} onChange={e => setOccasion(e.target.value)} className="w-full rounded-lg border px-3 py-2 text-sm mb-3 dark:border-gray-700 dark:bg-gray-800">
            <option value="wedding">👰 زفاف</option>
            <option value="birthday">🎂 عيد ميلاد</option>
            <option value="baby_shower">👶 بيبي شاور</option>
            <option value="other">🎁 مناسبة أخرى</option>
          </select>
          <input type="number" value={targetAmount} onChange={e => setTarget(Number(e.target.value))} placeholder="المبلغ المستهدف (ر.س)" className="w-full rounded-lg border px-3 py-2 text-sm mb-3 dark:border-gray-700 dark:bg-gray-800" />
          <Button onClick={() => { if(title.trim()) createMut.mutate({ title: title.trim(), occasion: occasion as 'birthday', targetAmount, serviceIds: [] }, { onSuccess: () => { setShowForm(false); setCreated(true); setTitle(''); } }); }} loading={createMut.isPending} className="w-full">🎁 إنشاء السجل</Button>
        </Card>}
