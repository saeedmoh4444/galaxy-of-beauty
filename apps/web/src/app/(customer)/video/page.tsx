    ...
    <DashboardLayout role="CUSTOMER">
      <div className="mx-auto max-w-3xl space-y-6">
        ...
        {isLoading ? <div className="space-y-3">{Array.from({length:3},(_,i)=><CardSkeleton key={i}/>)}</div> :
          bookings.length === 0 ? <Card padding="lg" className="text-center py-8"><p className="text-4xl mb-2">📹</p><p className="text-gray-500">مافي حجوزات حالية — احجزي خدمة لبدء استشارة فيديو</p><Link href="/customer/bookings/create"><Button className="mt-4">احجزي الآن</Button></Link></Card> :
          <div className="space-y-3">{bookings.map((b: Record<string,unknown>) => (
            <Card key={b.id as number} padding="md">
              <div className="flex items-center justify-between">
                ...
                <Link href={`/customer/video/${b.id}`}><Button size="sm">دخول</Button></Link>
              </div>
            </Card>
          ))}</div>
        }
