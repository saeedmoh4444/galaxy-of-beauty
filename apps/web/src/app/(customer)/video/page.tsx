  ...
  return (
    <DashboardLayout role="CUSTOMER">
      <div className="mx-auto max-w-3xl space-y-6">
        <div><h1 className="text-2xl font-bold">📹 استشارات الفيديو</h1><p className="mt-1 text-sm text-text-secondary">مكالمات فيديو مباشرة مع الفنيات</p></div>

        {isLoading ? <div className="space-y-3">{Array.from({length:3},(_,i)=><CardSkeleton key={i}/>)}</div> :
          bookings.length === 0 ? <Card padding="lg" className="text-center py-8"><p className="text-4xl mb-2">📹</p><p className="text-text-secondary">مافي حجوزات حالية — احجزي خدمة لبدء استشارة فيديو</p><Link href="/bookings/create"><Button className="mt-4">احجزي الآن</Button></Link></Card> :
              ...
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-text-secondary">{new Date(b.createdAt as string).toLocaleDateString('ar-SA')} · {b.status as string}</p>
                  </div>
                </div>
