...
import {
  Card, CardSkeleton, ErrorAlert, Button, Input, formatCurrency,
  PageContainer, PageTitle,
  BeautySavingsGoal, BeautyBudgetPlanner, LoyaltyDividendBadge,
  PriceAlertBadge, StudentDiscountBadge, LayawayBadge, BeautyBudgetCard,
  BeautySavingsMilestoneCard, TaxHelperCard,
} from '@galaxy/ui';
          ...
          <div className="space-y-6">
            ...
            <BeautyBudgetCard
              services={(budgetServices?.data?.items as any[])?.slice(0, 4)?.map((s: any) => ({
                name: (s.titleJson as any)?.ar ?? '', price: Number(s.basePrice),
                category: 'facial' as const, duration: `${s.durationMin} دقيقة`,
              })) ?? []}
            />
          </div>
            ...
            {savingsGoals?.data?.length > 0 && (
              <BeautySavingsGoal
                goals={(savingsGoals.data as any[]).slice(0, 3).map((g: any) => ({
                  label: g.name ?? 'هدف', target: g.amount ?? 0,
                  saved: g.saved ?? 0, monthly: g.monthly ?? 0,
                }))}
              />
            )}
