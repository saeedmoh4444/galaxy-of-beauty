...
export default function MySubscriptionPage(): JSX.Element {
  ...
  const { data: sub, isLoading, isError, refetch } = (api as any).subscriptions?.getMySubscription?.useQuery?.() as any;
  ...
  if (isLoading) return <DashboardLayout role="CUSTOMER"><CardSkeleton /></DashboardLayout>;
  if (isError) return <DashboardLayout role="CUSTOMER"><ErrorAlert message="فشل التحميل" onRetry={() => refetch()} /></DashboardLayout>;
  if (!sub) return <DashboardLayout role="CUSTOMER"><div className="mx-auto max-w-lg space-y-6"><h1 className="text-2xl font-bold">📦 اشتراكي</h1><EmptyState title="لا يوجد اشتراك نشط" description="اشتركي في باقة شهرية واحصلي على خدمات بتخفيض" /><div className="text-center"><Link href="/subscription-boxes"><Button>تصفحي الباقات</Button></Link></div></div></DashboardLayout>;

  const plan = sub.plan || {};
  const planName = ((plan as any).nameJson as Record<string,string>)?.ar || 'الباقة';
  const nextDate = sub.currentPeriodEnd ? new Date(sub.currentPeriodEnd).toLocaleDateString('ar-SA') : '—';
