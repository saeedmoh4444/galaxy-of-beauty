  ...
  return (
    <DashboardLayout role="CUSTOMER">
      <div className="mx-auto max-w-3xl space-y-6">
        <div><h1 className="text-2xl font-bold">🔔 تنبيهات الأسعار</h1><p className="mt-1 text-sm text-text-secondary">تابعي أسعار الخدمات واحصلي على تنبيه عند انخفاضها</p></div>
          ...
          <div className="grid gap-3 sm:grid-cols-3">{services.map((s: Record<string,unknown>) => (
            <div key={s.id as number} className="rounded-xl bg-green-50 dark:bg-green-950 p-3 text-center">
              ...
              <div className="flex items-center justify-center gap-2 mt-1">
                <span className="text-xs text-text-tertiary line-through">{formatCurrency(s.prevPrice as number)}</span>
        ...
        <Card padding="lg"><div className="flex items-center justify-between mb-4"><h3 className="font-bold">🔔 تنبيهاتي</h3><Button size="sm" onClick={() => setShow(true)}>+ إضافة</Button></div>
          {alerts.length === 0 ? <p className="text-sm text-text-tertiary text-center py-4">لا توجد تنبيهات</p> :
            <div className="space-y-2">{alerts.map((a: Record<string,unknown>) => (
              <div key={a.id as number} className="flex items-center justify-between rounded-lg bg-surface-muted dark:bg-gray-800 p-3"><div><span className="text-lg mr-2">{a.emoji as string}</span><span className="font-bold">{a.serviceName as string}</span></div><div className="flex items-center gap-3"><span className="text-sm">السعر المستهدف: {formatCurrency(a.targetPrice as number)}</span><button onClick={() => deleteMut.mutate({ id: a.id as number })} className="text-red-400">🗑️</button></div></div>
            ))}</div>
        }</Card>
