            ...
            <div className="flex flex-wrap gap-2 mb-4">
              {allInterests.map((i: Record<string, unknown>) => (
                <button
                  ...
                  className={`rounded-full px-4 py-2 text-sm font-medium transition-all ${selected.includes(i.key as string) ? 'bg-brand-600 text-white' : 'bg-surface-muted hover:bg-surface-muted'}`}
                >
          ...
          <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
            {pals.map((p: Record<string, unknown>) => (
              <Card key={p.userId as number} padding="lg" className="text-center">
                <div className="flex h-14 w-14 mx-auto items-center justify-center rounded-full bg-linear-to-br from-brand-400 to-brand-500 text-white text-xl font-bold">
