  ...
  return (
    <DashboardLayout role="CUSTOMER">
      <div className="mx-auto max-w-5xl space-y-6">
        <div><h1 className="text-2xl font-bold">💳 عضويات الصالون</h1><p className="mt-1 text-sm text-gray-500">اختاري العضوية اللي تناسبكِ</p></div>
        {isLoading ? <CardSkeleton/> : (membership?.tier as string) && <Card padding="lg" className="text-center border-2 border-purple-300"><p className="text-sm text-gray-500">عضويتكِ الحالية</p><p className="text-3xl font-extrabold text-purple-600 mt-1">{(membership?.tier as string)==='platinum'?'💎 بلاتينية':(membership?.tier as string)==='premium'?'🥇 مميزة':'🥈 أساسية'}</p>{(membership?.autoRenew as boolean) && <Button variant="ghost" onClick={()=>cancelMut.mutate()} className="mt-3 text-red-500">إلغاء التجديد التلقائي</Button>}</Card>}
        <div className="grid gap-6 lg:grid-cols-3">{MEMBERSHIPS.map(m=>(<Card key={m.key} padding="lg" className="text-center"><span className="text-5xl">{m.emoji}</span><h2 className="text-xl font-bold mt-2" style={{color:m.color}}>{m.name}</h2><p className="text-2xl font-extrabold mt-1">{m.price===0?'مجانية':formatCurrency(m.price)+' / شهرياً'}</p><div className="mt-4 space-y-2 text-sm text-right"><p className="font-semibold text-gray-700">✅ المميزات</p>{m.benefits.map((b,i)=><p key={i} className="text-green-600">✓ {b}</p>)}{m.notIncluded.length>0&&<><p className="font-semibold text-gray-400 mt-3">🚫 غير متضمن</p>{m.notIncluded.map((b,i)=><p key={i} className="text-gray-400">✗ {b}</p>)}</>}<Button onClick={()=>subscribeMut.mutate({tier:m.key,autoRenew:true})} loading={subscribeMut.isPending} className="w-full mt-4">{m.price===0?'مجانية':'اشتراك'}</Button></div></Card>))}</div>
      </div>
    </DashboardLayout>
  );
}
