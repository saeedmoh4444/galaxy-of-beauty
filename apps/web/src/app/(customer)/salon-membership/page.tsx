  ...
  return (
    <DashboardLayout role="CUSTOMER">
      <div className="mx-auto max-w-5xl space-y-6">
        ...
        {isLoading ? <CardSkeleton/> : membership?.tier&&<Card padding="lg" className="text-center border-2 border-purple-300"><p className="text-sm text-gray-500">عضويتكِ الحالية</p><p className="text-3xl font-extrabold text-purple-600 mt-1">{membership.tier==='platinum'?'💎 بلاتينية':membership.tier==='premium'?'🥇 مميزة':'🥈 أساسية'}</p>{membership.autoRenew&&<Button variant="ghost" onClick={()=>cancelMut.mutate()} className="mt-3 text-red-500">إلغاء التجديد التلقائي</Button>}</Card>}
