...
export default function StyleMatchPage(): JSX.Element {
  ...
  const addColor = () => { if (customColors.length < 5) setCustomColors([...customColors, '#C4A38C']); };
  const removeColor = (i: number) => { if (customColors.length > 1) setCustomColors(customColors.filter((_, idx) => idx !== i)); };
  const updateColor = (i: number, hex: string) => {
    const c = [...customColors]; c[i] = hex; setCustomColors(c);
  };
  const applyPreset = (colors: string[], label: string) => {
    setCustomColors(colors); setActivePreset(activePreset === label ? null : label);
  };
      ...
      <div className="mx-auto max-w-5xl space-y-6">
        ...
        <div className="grid gap-6 lg:grid-cols-2">
          ...
          <div className="space-y-4">
            ...
            <div>
              <Button onClick={handleMatch} loading={matchMut.isPending} className="w-full" size="lg">🔍 ابحثي عن إطلالات مشابهة</Button>
            </div>
          </div>
        ...
        {matchMut.isPending ? (
          ...
          <EmptyState title="لا توجد إطلالات مطابقة" description="جربي تغيير الألوان أو نوع الإطلالة" />
        ) : resultsList.length > 0 ? (
          <div>
            <h3 className="font-bold text-lg mb-4">✨ إطلالات مشابهة</h3>
            <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
              {resultsList.map((r: Record<string, unknown>) => (
                <Card key={r.id as number} padding="lg" className="group hover:shadow-xl transition-all">
                  <div className="flex h-36 items-center justify-center rounded-xl bg-gradient-to-br from-brand-100 to-purple-100 dark:from-brand-900 dark:to-purple-900 text-5xl">
                    {['daily','evening','party','bridal','summer','winter','traditional'].includes(r.category as string)
                      ? ({daily:'☀️',evening:'🌙',party:'🎉',bridal:'👰',summer:'🌴',winter:'❄️',traditional:'🧕'} as Record<string,string>)[r.category as string]
                      : '✨'}
                  </div>
                  <div className="mt-3">
                    ...
                    {(r.tutorialId as number) && (
                      <Link href={`/tutorials/${r.tutorialId as number}`} className="mt-3 inline-block">
                        <Button size="sm" variant="ghost">📹 شاهدي الدرس</Button>
                      </Link>
                    )}
                  </div>
                </Card>
