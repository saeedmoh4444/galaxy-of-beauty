    ...
    <DashboardLayout role="CUSTOMER">
      <div className="mx-auto max-w-3xl space-y-6">
        <div className="flex items-center justify-between">
          <div><h1 className="text-2xl font-bold">🛒 سلة التسوق</h1><p className="mt-1 text-sm text-gray-500">منتجات التجميل في سلتكِ</p></div>
        ...
        {isLoading ? <div className="space-y-3">{Array.from({length:3},(_,i)=><CardSkeleton key={i}/>)}</div> :
          cartItems.length === 0 ? <Card padding="lg" className="text-center py-8"><p className="text-4xl mb-2">🛒</p><p className="text-gray-500">سلتكِ فاضية — تصفحي المنتجات وأضيفي اللي يعجبكِ</p></Card> :
          <>
                  ...
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <p className="text-xs text-gray-500">الكمية: {item.quantity as number} · {formatCurrency(Number(product?.price ?? 0))}</p>
                      </div>
                    </div>
                    <div className="flex items-center gap-3">
            ...
            <Card padding="lg"><div className="flex justify-between items-center">
              <div><p className="text-sm text-gray-500">الإجمالي</p><p className="text-2xl font-extrabold">{formatCurrency(total)}</p></div>
