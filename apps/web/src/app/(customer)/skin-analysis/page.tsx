...
export default function SkinAnalysisPage(): JSX.Element {
  ...
  const { data: history, isLoading, isError, refetch } = api.skinAnalysis.history.useQuery({ page: 1, limit: 10 });
  ...
  const skinTypes = histItems.map((a) => (a.resultJson as Record<string, unknown>)?.skinType as string).filter(Boolean);
  const latestSkinType = skinTypes[0] ?? 'غير محدد';
  const allConcerns = histItems.flatMap((a) => ((a.resultJson as Record<string, unknown>)?.concerns as string[]) ?? []).filter(Boolean);
  ...
  return (
    <DashboardLayout role="CUSTOMER">
      <div className="mx-auto max-w-4xl space-y-6">
        ...
        {/* Result */}
        {result && (
          <Card padding="lg" className="border-2 border-brand-300 bg-brand-50/50 dark:border-brand-800 dark:bg-brand-950/50">
              ...
              <div className="mt-4">
                ...
                <div className="flex flex-wrap gap-2">
                  {(result['concerns'] as string[])?.map((c: string) => (
                    <span key={c} className="rounded-full bg-red-100 px-3 py-1 text-xs text-red-700 dark:bg-red-900 dark:text-red-300">{c}</span>
                  ))}
                </div>
              </div>
            ))}
            {(result['recommendations'] as Record<string, unknown>) && ((
              <div className="mt-4 space-y-3">
                ...
                {['services', 'products', 'routine'].map((cat) => {
                  const recs = (result['recommendations'] as Record<string, unknown>)?.[cat] as string[];
        ...
        {isLoading ? <CardSkeleton /> :
         ...
         histItems.length === 0 ? <EmptyState title="لا توجد تحليلات سابقة" description="حمّلي أول صورة لبشرتكِ للتحليل" /> :
         <div className="space-y-3">
           {histItems.map((a, idx) => {
             const res = a.resultJson as Record<string, unknown>;
             return (
               <Card key={a.id as number} padding="md" className={`flex items-center gap-4 ${idx === 0 ? 'border-l-4 border-brand-500' : ''}`}>
                 ...
                 <div className="flex-1 min-w-0">
                   <div className="flex items-center gap-2">
                     ...
                     {idx === 0 && <span className="rounded-full bg-brand-100 px-2 py-0.5 text-[10px] font-bold text-brand-700 dark:bg-brand-900 dark:text-brand-300">الأحدث</span>}
                   </div>
