...
export default function AiFeedPage(): JSX.Element {
  const { data: feed, isLoading } = api.aiFeatures.personalizedFeed.useQuery() as { data: Record<string,unknown> | undefined; isLoading: boolean };
  ...
  return (
    <DashboardLayout role="CUSTOMER">
      <div className="mx-auto max-w-4xl space-y-6">
        <div><h1 className="text-2xl font-bold">🤖 لكِ خصيصاً</h1><p className="mt-1 text-sm text-gray-500">توصيات ذكية مبنية على تفضيلاتكِ</p></div>
          ...
          <>
            ...
            {wishlistItems.length > 0 && <Card padding="lg"><h3 className="font-bold mb-3">💝 قائمة أمنياتكِ</h3><div className="grid gap-3 sm:grid-cols-2">{wishlistItems.map((w: Record<string,unknown>) => (
              <div key={w.id as number} className="flex items-center justify-between rounded-lg border p-3">
                <div><p className="font-bold text-sm">{(w.titleJson as Record<string,string>)?.ar ?? String(w.id)}</p><p className="text-xs text-gray-500">{w.imageUrl ? '📸 متوفر' : '📋 خدمة'}</p></div>
            ...
            {recommendations.length > 0 && <div>
              ...
              <div className="grid gap-4 sm:grid-cols-3">{recommendations.map((r: Record<string,unknown>) => (
                <Card key={r.id as number} padding="md" className="text-center">
                  ...
                  <p className="text-xs text-gray-500 mt-1">{(r.category as Record<string,unknown>)?.nameJson ? ((r.category as Record<string,unknown>)?.nameJson as Record<string,string>)?.ar : ''}</p>
                  <p className="font-bold text-brand-600 mt-2">{formatCurrency(r.basePrice as number)}</p>
                </Card>
              ))}</div>
            </div>}

            {wishlistItems.length === 0 && recommendations.length === 0 && <Card padding="lg" className="text-center py-8"><p className="text-4xl mb-2">🤖</p><p className="text-gray-500">احجزي خدمات أكثر علشان نقدر نقترح لكِ الأفضل</p></Card>}
          </>
        )}
      </div>
    </DashboardLayout>
