      ...
      <div className="mx-auto max-w-5xl space-y-6">
        ...
        {acctLoading ? <CardSkeleton/> : (
          <Card padding="lg" className={`text-center bg-gradient-to-r ${TIER_COLORS[tier] ?? TIER_COLORS['SILVER']!} text-white`}>
            ...
            {(account?.nextTier as Record<string,unknown>) && <p className="text-xs mt-2 bg-white dark:bg-gray-900/20 rounded-full px-3 py-1 inline-block">تحتاجين {(account!.nextTier as Record<string,unknown>).pointsNeeded as number} نقطة للوصول لـ {(account!.nextTier as Record<string,unknown>).name as string}</p>}
          </Card>
        )}
            ...
            return (
              <Card key={r.id as number} padding="lg" className={`text-center ${isRedeemed ? 'border-2 border-green-300 bg-green-50' : canAfford ? '' : 'opacity-50'}`}>
                ...
                {isRedeemed ? <p className="text-green-600 font-bold mt-3">✅ تم الاستبدال</p> :
                  <Button size="sm" onClick={() => redeemMut.mutate({ rewardId: r.id as number }, { onSuccess: () => setRedeemed(r.id as number) })} loading={redeemMut.isPending} disabled={!canAfford} className="w-full mt-3">{canAfford ? '🎁 استبدلي' : '🔒 نقاط غير كافية'}</Button>
                }
              </Card>
            );
          })}</div>
