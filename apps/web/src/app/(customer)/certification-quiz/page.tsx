...
export default function CertificationQuizPage(): JSX.Element {
  const { data: quizzes, isLoading } = api.certificationQuiz.quizzes.useQuery() as { data: Array<Record<string,unknown>> | undefined; isLoading: boolean };
  const { data: certs } = api.certificationQuiz.myCertificates.useQuery() as { data: Array<Record<string,unknown>> | undefined };
  const [quizId, setQuizId] = useState<string | null>(null);
  const { data: quiz } = api.certificationQuiz.get.useQuery({ id: quizId ?? '' }, { enabled: !!quizId }) as { data: Record<string,unknown> | undefined };
      ...
      <div className="mx-auto max-w-3xl space-y-6">
        ...
        {result ? (
          ...
          <Card padding="lg">
            ...
            <div className="space-y-4">{questions.map((q: Record<string,unknown>, qi: number) => (
              <div key={qi}><p className="font-semibold text-sm mb-2">{q.q as string}</p>
                <div className="space-y-1">{(q.opts as string[]).map((opt: string, oi: number) => (
                  <button key={oi} onClick={() => { const a = [...answers]; a[qi] = oi; setAnswers(a); }} className={`w-full text-right rounded-lg border p-3 text-sm transition-all ${answers[qi] === oi ? 'border-brand-400 bg-brand-50 dark:bg-brand-950' : 'border-gray-200 dark:border-gray-700'}`}>{opt}</button>
                ))}</div>
              </div>
            ))}</div>
            <Button onClick={() => submitMut.mutate({ quizId, answers }, { onSuccess: (d) => setResult(d as Record<string,unknown>) })} loading={submitMut.isPending} className="w-full mt-4">تقديم ✓</Button>
          </Card>
