...
export default function FollowingPage(): JSX.Element {
  const { data, isLoading } = api.technicianFollows.myFollows.useQuery() as { data: Array<Record<string,unknown>> | undefined; isLoading: boolean; isError: boolean; refetch: () => void };
  ...
  return (
    <DashboardLayout role="CUSTOMER">
      <div className="mx-auto max-w-3xl space-y-6">
        <div><h1 className="text-2xl font-bold">👩‍🎨 متابعة الفنيات</h1><p className="mt-1 text-sm text-text-secondary">الفنيات اللي تتابعينهم</p></div>

        {isLoading ? <div className="space-y-3">{Array.from({length:3},(_,i)=><CardSkeleton key={i}/>)}</div> :
          follows.length === 0 ? <Card padding="lg" className="text-center py-8"><p className="text-4xl mb-2">👩‍🎨</p><p className="text-text-secondary">مافي فنيات متابعات بعد</p></Card> :
              ...
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-text-secondary">تمت المتابعة {new Date(f.createdAt as string).toLocaleDateString('ar-SA')}</p>
                  </div>
                </div>
