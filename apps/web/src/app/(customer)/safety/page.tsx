      ...
      <PageContainer width="wide">
        ...
        <div className="grid gap-6 lg:grid-cols-3">
          <div className="lg:col-span-2 space-y-6">
            ...
            <LocationSharingCard
              contacts={(emergencyContacts?.data as any[])?.map((c: any) => ({ name: c.name, phone: c.phone, relation: c.relation })) ?? [{ name: 'أمي', phone: '0550000000', relation: 'mother' }]}
              address={booking?.address ?? 'الرياض'}
              technicianName={booking?.technician?.name ?? 'نورة'}
              estimatedEnd={booking?.endAt ? new Date(booking.endAt).toLocaleTimeString('ar-SA', { hour: '2-digit', minute: '2-digit' }) : '21:00'}
            />
          </div>
