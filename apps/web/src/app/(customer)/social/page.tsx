...
export default function SocialPage(): JSX.Element {
  ...
  const { data: tipsData, isLoading: tipsLoading } = api.social.tips.useQuery({ page: 1 }) as {
    data: Array<Record<string, unknown>> | undefined;
    isLoading: boolean;
    isError: boolean;
    refetch: () => void;
  };
  const { data: lookbook, isLoading: lbLoading } = api.social.lookbook.useQuery() as {
    data: Array<Record<string, unknown>> | undefined;
    isLoading: boolean;
    isError: boolean;
    refetch: () => void;
  };
  const { data: feedData, isLoading: feedLoading } = api.social.feed.useQuery({
    page: 1,
    limit: 12,
  }) as {
  ...
  const tips = (tipsData ?? []).filter(
    (t: Record<string, unknown>) => tipCat === 'all' || (t.category as string) === tipCat,
  );
              ...
              {feedItems.map((f: Record<string, unknown>) => (
                <div key={f.id as number} className="rounded-lg border p-2 text-center">
                  ...
                  <p className="text-xs text-text-tertiary">
                    {(((f.technician as Record<string, unknown>)?.user as Record<string, unknown>)
                      ?.name as string) ?? ''}
                  </p>
                </div>
              ))}
