...
export default function SalonManagementPage(): JSX.Element {
  const { data: dash } = api.salonManagement.dashboard.useQuery() as { data: Record<string,unknown> | undefined };
  const { data: staff, refetch } = api.salonManagement.staff.useQuery() as { data: Array<Record<string,unknown>> | undefined; refetch: () => void };
  const addMut = api.salonManagement.addStaff.useMutation({ onSuccess: () => { setShow(false); refetch(); } });
  ...
  return (
    <DashboardLayout role="CUSTOMER">
      <div className="mx-auto max-w-4xl space-y-6">
        ...
        <Modal open={show} onClose={() => setShow(false)} title="إضافة موظفة"><div className="space-y-3">
          <input value={name} onChange={(e) => setName(e.target.value)} placeholder="الاسم" className="w-full rounded-lg border px-3 py-2 text-sm dark:border-gray-700 dark:bg-gray-800" />
          <input value={role} onChange={(e) => setRole(e.target.value)} placeholder="الدور" className="w-full rounded-lg border px-3 py-2 text-sm dark:border-gray-700 dark:bg-gray-800" />
          <Button onClick={() => { if (name.trim() && role.trim()) addMut.mutate({ name: name.trim(), role: role.trim() }); }} loading={addMut.isPending} className="w-full">👩‍🎨 إضافة</Button>
        </div></Modal>
      </div>
    </DashboardLayout>
  );
