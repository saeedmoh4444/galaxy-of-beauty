            ...
            {COMMON_ALLERGENS.map((a) => {
              ...
              return (
                <button
                  ...
                  className={`rounded-xl border-2 p-3 text-end transition-all ${isChecked ? (a.risk === 'high' ? 'border-red-400 bg-red-50' : a.risk === 'medium' ? 'border-amber-400 bg-amber-50' : 'border-green-400 bg-green-50 dark:bg-green-950') : 'border-edge'}`}
                >
                  <span className="text-2xl">{a.emoji}</span>
                  <span className="font-bold ms-2">{t(a.name)}</span>
                  <span
                    className={`text-xs rounded-full px-2 py-0.5 ${a.risk === 'high' ? 'bg-red-100 dark:bg-red-950 text-red-700 dark:text-red-300' : a.risk === 'medium' ? 'bg-amber-100 dark:bg-amber-950 text-amber-700 dark:text-amber-300' : 'bg-green-100 dark:bg-green-950 text-green-700 dark:text-green-300'}`}
                  >
                    {a.risk === 'high'
