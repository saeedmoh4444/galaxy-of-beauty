  ...
  return (
    <DashboardLayout role="CUSTOMER">
      <div className="mx-auto max-w-3xl space-y-6">
        <div><h1 className="text-2xl font-bold">🔮 توصيات</h1><p className="mt-1 text-sm text-text-secondary">اكتشفي خدمات تناسب ذوقكِ</p></div>

        <Card padding="lg"><h3 className="font-bold mb-3">💅 اختاري خدمة</h3>
          <div className="flex flex-wrap gap-2">{POPULAR_SERVICES.map(s => (
            <button key={s.id} onClick={()=>setServiceId(s.id)} className={`rounded-full px-4 py-2 text-sm transition-all ${serviceId===s.id?'bg-brand-600 text-white':'bg-surface-muted'}`}>{s.emoji} {s.name}</button>
          ))}</div>
        </Card>

        <Card padding="lg"><h3 className="font-bold mb-4">🔗 تحجز مع بعض</h3>
          {tLoading ? <CardSkeleton/> : !(together??[]).length ? <p className="text-sm text-text-tertiary">لا توجد توصيات حالياً</p> :
            <div className="grid gap-3 sm:grid-cols-2">{(together??[]).map((s: Record<string,unknown>) => (
              <div key={s.id as number} className="flex items-center justify-between rounded-lg border p-3">
                <div><p className="font-bold text-sm">{s.title as string}</p><p className="text-xs text-text-secondary">{formatCurrency(s.basePrice as number)}</p></div>
        ...
        <Card padding="lg"><h3 className="font-bold mb-4">✨ كملي الطلة</h3>
          {cLoading ? <CardSkeleton/> : !(complete??[]).length ? <p className="text-sm text-text-tertiary">لا توجد توصيات حالياً</p> :
            <div className="grid gap-3 sm:grid-cols-2">{(complete??[]).map((s: Record<string,unknown>) => (
              <div key={s.id as number} className="rounded-lg border p-3">
                ...
                <p className="text-xs text-text-secondary mt-1">{formatCurrency(s.basePrice as number)} · {s.durationMin as number} دقيقة</p>
