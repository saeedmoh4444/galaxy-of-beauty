...
export default function SkinDiaryPage(): JSX.Element {
  const { data: entries, isLoading, isError, refetch } = api.skinDiary.entries.useQuery() as {
    data: Array<Record<string,unknown>> | undefined; isLoading: boolean; isError: boolean; refetch: () => void;
  };
  const { data: timeline } = api.skinDiary.timeline.useQuery() as { data: Array<Record<string,unknown>> | undefined };
  const addMut = api.skinDiary.add.useMutation({ onSuccess: () => { setShowAdd(false); refetch(); } });
  ...
  return (
    <DashboardLayout role="CUSTOMER">
      <div className="mx-auto max-w-4xl space-y-6">
        ...
        {timelineData.length > 1 && (
          <Card padding="lg">
            ...
            <div className="flex items-end gap-1 h-24">
              {timelineData.slice(0, 14).reverse().map((d: Record<string,unknown>) => {
        ...
        : items.length === 0 ? <EmptyState title="لا توجد إدخالات" description="أضيفي أول صورة لبشرتكِ لبدء تتبع التحسن" action={{ label: 'إضافة', onPress: () => setShowAdd(true) }} />
        ...
        <Modal open={showAdd} onClose={() => setShowAdd(false)} title="إضافة للبوميات">
          <div className="space-y-3">
            <div><label className="text-sm font-semibold">رابط الصورة</label><input type="url" value={imageUrl} onChange={(e) => setImageUrl(e.target.value)} className="w-full rounded-lg border px-3 py-2 text-sm dark:border-gray-700 dark:bg-gray-800 mt-1" /></div>
            <div><label className="text-sm font-semibold">حالة البشرة</label><select value={condition} onChange={(e) => setCondition(e.target.value)} className="w-full rounded-lg border px-3 py-2 text-sm dark:border-gray-700 dark:bg-gray-800 mt-1">{CONDITIONS.map((c) => <option key={c}>{c}</option>)}</select></div>
            <div><label className="text-sm font-semibold">الترطيب: {hydration}/10</label><input type="range" min={1} max={10} value={hydration} onChange={(e) => setHydration(parseInt(e.target.value))} className="w-full accent-brand-600 mt-1" /></div>
            <div><label className="text-sm font-semibold">ملاحظات</label><textarea value={notes} onChange={(e) => setNotes(e.target.value)} className="w-full rounded-lg border px-3 py-2 text-sm dark:border-gray-700 dark:bg-gray-800 mt-1" rows={2} /></div>
            <Button onClick={() => { if (imageUrl) addMut.mutate({ imageUrl, skinCondition: condition, hydration, notes: notes || undefined }); }} loading={addMut.isPending} className="w-full">💾 حفظ</Button>
          </div>
        </Modal>
      </div>
    </DashboardLayout>
