    ...
    <DashboardLayout role="CUSTOMER">
      <div className="mx-auto max-w-3xl space-y-6">
        <div className="flex items-center justify-between">
          <div><h1 className="text-2xl font-bold">⏰ تذكيرات الجمال</h1><p className="mt-1 text-sm text-text-secondary">لا تنسي مواعيد عنايتكِ</p></div>
        ...
        {showForm && <Card padding="lg">
          ...
          <div className="flex gap-2 mb-3 flex-wrap">{INTERVALS.map(d => <button key={d} onClick={() => setInterval(d)} className={`rounded-full px-3 py-1 text-xs ${interval===d?'bg-brand-600 text-white':'bg-surface-muted'}`}>كل {d} يوم</button>)}</div>
        ...
        {isLoading ? <div className="space-y-3">{Array.from({length:3},(_,i)=><CardSkeleton key={i}/>)}</div> :
          overdue.length === 0 && upcoming.length === 0 ? <Card padding="lg" className="text-center py-8"><p className="text-4xl mb-2">⏰</p><p className="text-text-secondary">مافي تذكيرات — أضيفي أول تذكير</p></Card> :
          <>
            ...
            {upcoming.length > 0 && <Card padding="lg"><h3 className="font-bold mb-3">📅 قادمة</h3>
              <div className="space-y-2">{upcoming.map((r: Record<string,unknown>) => (
                <div key={r.id as number} className="flex items-center justify-between rounded-lg border p-3">
                  <div className="flex items-center gap-3"><span className="text-2xl">{CATEGORIES[r.category as string]?.split(' ')[0]}</span><div><p className="font-bold text-sm">{r.title as string}</p><p className="text-xs text-text-secondary">{new Date(r.nextDate as string).toLocaleDateString('ar-SA')} · كل {r.intervalDays as number} يوم</p></div></div>
