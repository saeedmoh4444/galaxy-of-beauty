...
export default function ChatPage(): JSX.Element {
  ...
  const { data: messages, isLoading: msgLoading } = api.chat.messages.useQuery(
    { bookingId: selectedConv ?? 0 },
  ...
  const selectedMsgs = msgs.filter((m: Record<string, unknown>) => {
    ...
    return (
      m.bookingId === selectedConv || m.senderId === selectedConv || m.receiverId === selectedConv
    );
  });
  ...
  const handleSend = (receiverId: number, bookingId?: number) => {
    ...
    sendMut.mutate(
      { receiverId, bookingId, content: content.trim() },
                  ...
                  <input
                    ...
                    onKeyDown={(e) => {
                      if (e.key === 'Enter' && content.trim()) {
                        ...
                        if (conv)
                          handleSend(
                            (conv.otherParty as Record<string, unknown>)?.id as number,
                            selectedConv,
                          );
                      }
                    }}
