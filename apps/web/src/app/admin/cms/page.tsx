  ...
  return (
    <DashboardLayout role="ADMIN">
      <div className="mx-auto max-w-6xl space-y-6">
        <div><h1 className="text-2xl font-bold">📝 إدارة المحتوى</h1><p className="mt-1 text-sm text-gray-500">إدارة الفئات والخدمات</p></div>

        <div className="flex gap-2">
          <button onClick={() => setTab('categories')} className={`rounded-lg px-4 py-2 text-sm ${tab==='categories'?'bg-brand-600 text-white':'bg-gray-100'}`}>📂 الفئات</button>
          <button onClick={() => setTab('services')} className={`rounded-lg px-4 py-2 text-sm ${tab==='services'?'bg-brand-600 text-white':'bg-gray-100'}`}>💅 الخدمات</button>
        </div>
          ...
          <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">{(categories??[]).map((c: Record<string,unknown>) => (
            <Card key={c.id as number} padding="md">
              <div className="flex items-center gap-3">
                ...
                <div><p className="font-bold">{(c.nameJson as Record<string,string>)?.ar}</p><p className="text-xs text-gray-500">{c.slug as string} · {(c._count as Record<string,number>)?.services ?? 0} خدمة</p></div>
              </div>
            </Card>
          ))}</div>
        )}
          ...
          <div className="space-y-2">{(services??[]).map((s: Record<string,unknown>) => (
            <Card key={s.id as number} padding="md">
              <div className="flex items-center justify-between">
                <div><p className="font-bold">{(s.titleJson as Record<string,string>)?.ar}</p><p className="text-xs text-gray-500">{((s.category as Record<string,unknown>)?.nameJson as Record<string,string>)?.ar ?? ''} · {s.durationMin as number} دقيقة</p></div>
