...
export default function AdminBookingsPage(): JSX.Element {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-wrap gap-2">{STATUSES.map((s) => <button key={s} onClick={() => setStatus(s === 'ALL' ? undefined : s)} className={`rounded-full px-4 py-1.5 text-sm font-medium ${(s === 'ALL' && !status) || s === status ? 'bg-brand-600 text-white' : 'bg-surface-muted dark:bg-gray-800'}`}>{s}</button>)}</div>
      ...
      : <div className="space-y-2">{bookings.map((b: any) => (
          <Card key={b.id} padding="md">
            <div className="flex items-center justify-between">
              <div><p className="font-semibold">{b.bookingCode}</p><p className="text-sm text-text-secondary">{new Date(b.createdAt).toLocaleDateString('ar-SA')}</p></div>
