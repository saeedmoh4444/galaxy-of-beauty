  ...
  return (
    <DashboardLayout role="ADMIN">
      <div className="mx-auto max-w-4xl space-y-6">
        <div><h1 className="text-2xl font-bold">👥 الحجوزات الجماعية</h1><p className="mt-1 text-sm text-text-secondary">إدارة حجوزات المجموعات</p></div>

        {isLoading ? <div className="space-y-3">{Array.from({length:3},(_,i)=><CardSkeleton key={i}/>)}</div> :
          groups.length === 0 ? <Card padding="lg" className="text-center py-8"><p className="text-4xl mb-2">👥</p><p className="text-text-secondary">لا توجد حجوزات جماعية</p></Card> :
          <div className="space-y-3">{groups.map((g: Record<string,unknown>) => (
            <Card key={g.id as number} padding="md">
              <div className="flex items-center justify-between">
                <div><p className="font-bold">{g.name as string}</p><p className="text-xs text-text-secondary">{g.theme as string ?? ''} · {g.memberCount as number ?? (g._count as Record<string,number>)?.members} أعضاء</p></div>
