        ...
        <>
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            ...
            <Card padding="lg"><h3 className="font-bold mb-4">📍 حسب المدينة</h3>
              <div className="space-y-2">{byCity.map((c: Record<string,unknown>, i: number) => <div key={i} className="flex justify-between text-sm"><span>{c.city as string}</span><span className="text-text-secondary">{c.bookings as number} حجز</span><span className="font-bold">{formatCurrency(c.revenue as number)}</span></div>)}</div>
            </Card>
          </div>
        </>
      )}
