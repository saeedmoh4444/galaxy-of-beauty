...
export default function AdminPackagesPage(): JSX.Element {
  ...
  const createMut = api.beautyPackages.create.useMutation({ onSuccess: () => { refetch(); setShowCreate(false); } });
    ...
    <div className="space-y-6">
      ...
      <Modal open={showCreate} onClose={() => setShowCreate(false)} title="إضافة باقة جديدة">
        <div className="space-y-3">
          ...
          <Input label="نسبة الخصم" type="number" value={form.discountPercent} onChange={(e) => setForm({ ...form, discountPercent: Number(e.target.value) })} />
          <Input label="معرفات الخدمات (مفصولة بفواصل)" value={form.serviceIds} onChange={(e) => setForm({ ...form, serviceIds: e.target.value })} placeholder="1,2,3" />
          <Button onClick={() => createMut.mutate({ ...form, serviceIds: form.serviceIds.split(',').map(Number).filter(n => n > 0) })} loading={createMut.isPending}>حفظ</Button>
        </div>
      </Modal>
    </div>
  );
