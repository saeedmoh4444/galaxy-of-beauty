      ...
      {isLoading ? <div className="grid gap-4 sm:grid-cols-4">{Array.from({length:4},(_,i)=><CardSkeleton key={i}/>)}</div> :
        <>
          <div className="grid gap-4 sm:grid-cols-4">
            <Card padding="lg" className="text-center"><p className="text-3xl">💰</p><p className="text-2xl font-bold text-brand-600">{formatCurrency((d.revenue as Record<string,number>)?.today ?? 0)}</p><p className="text-xs text-gray-500">إيراد اليوم</p></Card>
            <Card padding="lg" className="text-center"><p className="text-3xl">📅</p><p className="text-2xl font-bold">{(d.bookings as Record<string,number>)?.today ?? 0}</p><p className="text-xs text-gray-500">حجز اليوم</p></Card>
            <Card padding="lg" className="text-center"><p className="text-3xl">👥</p><p className="text-2xl font-bold">{(d.users as Record<string,number>)?.activeToday ?? 0}</p><p className="text-xs text-gray-500">مستخدم نشط</p></Card>
            <Card padding="lg" className="text-center"><p className="text-3xl">👩‍🎨</p><p className="text-2xl font-bold">{(d.technicians as Record<string,number>)?.active ?? 0}</p><p className="text-xs text-gray-500">فنية نشطة</p></Card>
          </div>
          <Card padding="lg"><h3 className="font-bold mb-4">📈 الإيرادات الأسبوعية</h3>
            <div className="flex items-end gap-2 h-32">{(d.revenue as Record<string,number[]>)?.chart?.map((v: number, i: number) => <div key={i} className="flex-1 flex flex-col items-center gap-1"><div className="w-full rounded-t bg-gradient-to-t from-brand-400 to-brand-600" style={{ height: `${Math.max(4, (v / 600) * 100)}%` }} /><span className="text-[9px] text-gray-400">{v}</span></div>)}</div>
          </Card>
