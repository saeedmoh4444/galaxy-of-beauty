...
export default function TrendingPage(): JSX.Element {
  ...
  return (
    <div className="mx-auto max-w-5xl px-4 py-8 space-y-6">
      <div className="text-center"><h1 className="text-3xl font-bold">🔥 الأكثر رواجاً</h1><p className="mt-2 text-gray-500">أكثر الخدمات طلباً والفنيات تميزاً</p></div>
          ...
          {trLoading ? <CardSkeleton/> : !(trending??[]).length ? <p className="text-sm text-gray-400">لا توجد بيانات</p> :
            <div className="space-y-2">{(trending??[]).slice(0,10).map((s: Record<string,unknown>) => (
              <div key={s.serviceId as number} className="flex items-center justify-between rounded-lg border p-3">
                <div><p className="font-bold text-sm">{(s.titleJson as Record<string,string>)?.ar ?? `خدمة #${s.serviceId}`}</p><p className="text-xs text-gray-500">{formatCurrency(Number(s.basePrice ?? 0))}</p></div>
          ...
          {spLoading ? <CardSkeleton/> : !(spotlight??[]).length ? <p className="text-sm text-gray-400">لا توجد بيانات</p> :
            <div className="space-y-3">{(spotlight??[]).map((t: Record<string,unknown>) => (
              <div key={t.id as number} className="flex items-center gap-4 rounded-lg border p-3">
                ...
                <div><p className="font-bold">{t.name as string}</p><p className="text-xs text-gray-500">{t.city as string} · ⭐{t.ratingAvg as number}</p></div>
              </div>
            ))}</div>
          }
        </Card>
