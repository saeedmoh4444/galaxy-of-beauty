...
function GiftQuizWidget(): JSX.Element {
  ...
  return (
    <div className="mt-16 text-center">
      ...
      <p className="mt-2 text-gray-500">جاوبي على الأسئلة وبنقترح عليكِ أفضل الهدايا</p>
      ...
      {qLoading ? <CardSkeleton /> : done && results ? (
        <div className="mt-6">
          <div className="grid gap-4 sm:grid-cols-2">{results.slice(0,4).map((r: Record<string,unknown>) => (
            <Card key={r.id as number} padding="lg" className="text-center">
              ...
              <div className="mt-2 h-2 bg-gray-100 rounded-full"><div className="h-2 bg-brand-600 rounded-full" style={{width:`${r.score as number}%`}}/></div>
            </Card>
          ))}</div>
        ...
        <Card padding="lg" className="mt-6 max-w-lg mx-auto">
          ...
          <div className="space-y-2">{(currentQ.options as Array<Record<string,unknown>>).map((o: Record<string,unknown>) => (
            <button key={o.key as string} onClick={() => { setAnswers(prev => ({ ...prev, [currentQ.id as string]: o.key as string })); if (step < qs.length - 1) setStep(step + 1); }} className="w-full rounded-xl border-2 border-gray-200 p-3 text-right hover:border-brand-400 hover:bg-brand-50 transition-all">{o.labelAr as string}</button>
          ))}</div>
        </Card>
      ) : null}
    </div>
