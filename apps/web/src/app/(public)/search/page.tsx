...
export default function SearchPage(): JSX.Element {
  ...
  const { data: services, isLoading: svcLoading } = (api as any).services?.list?.useQuery?.({ search: query || undefined, limit: 12 }, { enabled: searched && query.length > 1 }) as any;
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  const { data: products, isLoading: prodLoading } = (api as any).marketplace?.products?.useQuery?.({ search: query || undefined, limit: 8 }, { enabled: searched && query.length > 1 }) as any;
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  const { data: technicians, isLoading: techLoading } = (api as any).technicians?.list?.useQuery?.({}, { enabled: searched && query.length > 1 }) as any;

  const handleSearch = () => { if (query.trim().length > 1) setSearched(true); };
  const svcItems = services?.items ?? [];
  const prodItems = products?.items ?? [];
  const techItems = Array.isArray(technicians) ? technicians.filter((t: any) => (t as any).user?.name?.includes?.(query)) : [];
  ...
  return (
    <div className="mx-auto max-w-5xl px-4 py-8">
      ...
      <div className="mx-auto mb-8 flex max-w-xl gap-2">
        ...
        <Button onClick={handleSearch} loading={isLoading}>بحث</Button>
      </div>
