...
export default function GroupBuyPage(): JSX.Element {
  ...
  return (
    <div className="mx-auto max-w-4xl px-4 py-12">
      <div className="mb-8 text-center"><span className="text-6xl">🛒</span><h1 className="mt-4 text-3xl font-bold">صفقات المجموعة</h1><p className="mt-2 text-gray-500">انضمي لمجموعة ووفري أكثر!</p></div>
      {isLoading ? <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">{Array.from({length:3},(_,i)=><CardSkeleton key={i}/>)}</div> :
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">{deals.map((d: Record<string,unknown>) => (
          <Card key={d.id as number} padding="lg" className="text-center border-2 border-amber-200 dark:border-amber-800">
            ...
            <div className="mt-2"><span className="text-gray-400 line-through">{formatCurrency(d.originalPrice as number)}</span><span className="text-2xl font-extrabold text-green-600 ml-2">{formatCurrency(d.groupPrice as number)} ر.س</span></div>
            ...
            <p className="text-xs text-gray-500 mt-1">{d.currentBuyers as number}/{d.minBuyers as number} مشتركة · ⏰ {d.endsIn as string}</p>
