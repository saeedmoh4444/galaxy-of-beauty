...
import { Card, ErrorAlert, EmptyState, ServiceImage } from '@galaxy/ui';
import { useLocale } from '@/components/LocaleProvider';

export interface BarberettesPageData {
  barberettes: Array<Record<string, unknown>>;
  fetchError?: string;
}

export function BarberettesClient({ data }: { data: BarberettesPageData }): JSX.Element {
  const { t } = useLocale();
  ...
  return (
    <div className="mx-auto max-w-5xl space-y-6 px-4 py-8">
      ...
      {data.barberettes.length === 0 ? (
        <EmptyState title={t('barberettes.empty')} />
      ) : (
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
          {data.barberettes.map((tr: Record<string, unknown>) => {
