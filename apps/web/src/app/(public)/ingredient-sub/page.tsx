  ...
  return (
    <div className="mx-auto max-w-3xl px-4 py-12">
      ...
      {q && isLoading ? <CardSkeleton /> : subs.length > 0 ? (
        <Card padding="lg"><h3 className="font-bold mb-3">🔄 بدائل {result?.ingredient as string}</h3><div className="space-y-3">{subs.map((s: Record<string,unknown>, i: number) => <div key={i} className="flex items-center gap-3 rounded-xl bg-green-50 dark:bg-green-950 p-4"><span className="text-3xl">{s.emoji as string}</span><div><p className="font-bold">{s.nameAr as string}</p><p className="text-xs text-text-secondary">{s.nameEn as string}</p><p className="text-xs text-gray-600 mt-1">{s.descAr as string}</p></div></div>)}</div></Card>
      ) : items.length > 0 ? (
