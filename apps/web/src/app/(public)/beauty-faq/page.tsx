...
export default function BeautyFaqPage(): JSX.Element {
  ...
  return (
    <div className="mx-auto max-w-3xl px-4 py-12">
      <div className="mb-8 text-center"><span className="text-6xl">🤖</span><h1 className="mt-4 text-3xl font-bold">Beauty FAQ</h1><p className="mt-2 text-gray-500">أجوبة على أسئلتكِ عن الجمال والعناية</p></div>
      ...
      <div className="flex flex-wrap gap-2 mb-6">
        <button onClick={() => setCategory(undefined)} className={`rounded-full px-3 py-1 text-xs ${!category ? 'bg-brand-600 text-white' : 'bg-gray-100 dark:bg-gray-800'}`}>الكل</button>
        {categories.map((c: Record<string,unknown>) => (
          <button key={c.key as string} onClick={() => setCategory(c.key as string)} className={`rounded-full px-3 py-1 text-xs ${category === c.key ? 'bg-brand-600 text-white' : 'bg-gray-100 dark:bg-gray-800'}`}>{c.emoji as string} {c.nameAr as string}</button>
        ))}
      </div>
          ...
          {faqs.map((f: Record<string,unknown>, i: number) => (
            <Card key={i} padding="lg">
              <details className="group">
                <summary className="cursor-pointer font-bold text-gray-900 dark:text-gray-100 hover:text-brand-600 transition-colors">{f.q as string}</summary>
