...
export default function ChallengesPage(): JSX.Element {
  const { data: challenges, isLoading } = api.challenges.list.useQuery() as { data: Array<Record<string,unknown>> | undefined; isLoading: boolean };

  return (
    <div className="mx-auto max-w-3xl px-4 py-8 space-y-6">
      <div className="text-center"><h1 className="text-3xl font-bold">🏆 تحديات الجمال</h1><p className="mt-2 text-gray-500">أكملي التحديات واكسبي مكافآت حصرية</p></div>

      {isLoading ? <div className="space-y-4">{Array.from({length:4},(_,i)=><CardSkeleton key={i}/>)}</div> :
        !(challenges??[]).length ? <Card padding="lg" className="text-center py-8"><p className="text-4xl mb-2">🏆</p><p className="text-gray-500">لا توجد تحديات حالياً</p></Card> :
          ...
          <Card key={c.key as string} padding="lg">
            <div className="flex items-start gap-4">
              ...
              <div className="flex-1">
                ...
                <p className="text-sm text-gray-500 mt-1">{c.desc as string ?? ''}</p>
