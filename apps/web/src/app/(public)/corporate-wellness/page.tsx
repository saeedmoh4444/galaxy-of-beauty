...
export default function CorporateWellnessPage(): JSX.Element {
  ...
  return (
    <div className="mx-auto max-w-5xl px-4 py-12">
      <div className="mb-8 text-center"><span className="text-6xl">🏢</span><h1 className="mt-4 text-3xl font-bold">برنامج الشركات</h1><p className="mt-2 text-gray-500">باقات تجميل وعناية لمنسوبات الشركات</p></div>
      {isLoading ? <div className="grid gap-6 lg:grid-cols-3">{Array.from({length:3},(_,i)=><CardSkeleton key={i}/>)}</div> :
        <div className="grid gap-6 lg:grid-cols-3">{list.map((p: Record<string,unknown>) => (
          <Card key={p.id as string} padding="lg" className="text-center hover:shadow-xl transition-all">
            ...
            <p className="text-3xl font-extrabold text-brand-600 mt-3">{formatCurrency(p.price as number)} ر.س<span className="text-sm text-gray-400 font-normal"> / شهرياً</span></p>
            <p className="text-sm text-gray-500 mt-2">حتى {(p.employees as number)} موظفة</p>
      ...
      {done ? <Card padding="lg" className="mt-6 text-center border-2 border-green-300"><span className="text-5xl">✅</span><p className="text-xl font-bold mt-2">تم استلام طلبكِ</p><p className="text-gray-500">سنتواصل معكِ خلال ٢٤ ساعة</p></Card> : null}
