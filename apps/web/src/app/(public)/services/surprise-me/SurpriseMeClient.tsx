...
import { Button, Card, CardSkeleton, ErrorAlert, ar } from '@galaxy/shared';
...
export function SurpriseMeClient({ data }: { data: SurpriseMePageData }): JSX.Element {
  ...
  const pickRandom = async () => {
    ...
    try {
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      const svcQuery = api.services.list.useQuery({ sort: 'popular', page: 1, limit: 50 }) as any;
      // Wait briefly for the query — in production this would use api.services.list.fetch()
      const utils = api.useUtils();
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      const result = await (utils as any).services.list.fetch({ sort: 'popular', page: 1, limit: 50 });
  ...
  return (
    <div className="mx-auto max-w-2xl px-4 py-16 text-center">
      ...
      {loading ? (
        <div className="mx-auto max-w-sm"><CardSkeleton /></div>
      ) : error ? (
