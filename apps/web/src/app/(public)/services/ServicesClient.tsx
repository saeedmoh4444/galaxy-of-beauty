        ...
        {cats.length > 0 && (
          <div className="flex flex-wrap gap-2">
            {cats.map((c: AnyRecord) => (
              <Link key={c.id} href={`/services?categoryId=${c.id}`} className="rounded-full bg-gray-100 px-3 py-1 text-xs dark:bg-gray-800">
                {ar(c.nameJson)}
              </Link>
            ))}
          </div>
            ...
            compareMode ? (
              <button key={svc.id} onClick={() => toggleSelect(svc.id)} className={`text-right ${selected.has(svc.id) ? 'ring-2 ring-brand-500 rounded-2xl' : ''}`}>
                <Card hover padding="md" className="relative">
                  ...
                  <h3 className="mt-3 font-semibold text-gray-900 dark:text-gray-100">{ar(svc.titleJson)}</h3>
