  ...
  return (
    <div className="mx-auto max-w-2xl px-4 py-12">
      <div className="mb-8 text-center">
        <ServiceImage
          service={pageHeroKey('audio-rooms')}
          alt=""
          size="xl"
      ...
      {isLoading ? (
        ...
        <>
          {live.length > 0 && (
            <h3 className="font-bold mb-3">{t('marketing.audio-rooms.live-now')}</h3>
          )}
              ...
              <div className="flex items-center gap-4">
                ...
                <div className="flex-1">
                  ...
                  <p className="text-xs text-text-secondary">
                    {r.host as string} ·{' '}
                    {t('marketing.audio-rooms.listeners', { count: r.listeners as number })}
                  </p>
                </div>
                {user && (
                  <Button size="sm" onClick={() => joinMut.mutate({ roomId: r.id as number })}>
                    {t('marketing.audio-rooms.join')}
                  </Button>
                )}
              </div>
            </Card>
          ...
          {upcoming.length > 0 && (
            <h3 className="font-bold mb-3 mt-6">{t('marketing.audio-rooms.upcoming')}</h3>
          )}
            ...
            <Card key={r.id as number} padding="lg" className="mb-3 opacity-70">
              <div className="flex items-center gap-4">
                ...
                <div className="flex-1">
                  ...
                  <p className="text-xs text-text-secondary">
                    {r.host as string} ·{' '}
                    {new Date(r.scheduledFor as string).toLocaleDateString(
