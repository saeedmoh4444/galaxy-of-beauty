  ...
  return (
    <div className="mx-auto max-w-6xl px-4 py-12">
      <div className="mb-10 text-center">
        ...
        <p className="mt-2 text-gray-500 dark:text-gray-400">باقات مجمعة بأسعار مخفضة — وفرّي عند حجز أكثر من خدمة معاً</p>
      </div>
      {packages.length === 0 ? (
        <div className="py-16 text-center text-gray-400">
          <span className="text-5xl">💅</span>
          <p className="mt-4">لا توجد باقات حالياً. تابعينا قريباً!</p>
          <Link href="/services" className="mt-4 inline-block text-brand-600 hover:underline">تصفحي جميع الخدمات</Link>
        </div>
      ) : (
        <div className="grid gap-8 sm:grid-cols-2 lg:grid-cols-3">
          {packages.map((pkg: Record<string, any>) => {
            ...
            return (
              <div key={pkg.id} className="overflow-hidden rounded-2xl border border-gray-200 bg-white dark:border-gray-800 dark:bg-gray-900">
                <div className="flex h-40 items-center justify-center bg-gradient-to-br from-brand-200 to-accent-200 text-5xl dark:from-brand-900 dark:to-accent-900">
                  {pkg.imageUrl ? <img src={pkg.imageUrl} alt={name} className="h-full w-full object-cover" /> : <span>💅</span>}
                </div>
                <div className="p-5">
                  <div className="flex items-center justify-between">
                    <h2 className="text-lg font-bold text-gray-900 dark:text-gray-100">{name}</h2>
                    <span className="rounded-full bg-red-100 px-2 py-0.5 text-xs font-bold text-red-600">-{pkg.discountPercent}%</span>
                  </div>
