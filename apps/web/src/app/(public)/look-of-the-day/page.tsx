...
export default function LookOfTheDayPage(): JSX.Element {
  ...
  return (
    <div className="mx-auto max-w-5xl px-4 py-12">
      <div className="mb-8 text-center"><span className="text-6xl">📸</span><h1 className="mt-4 text-3xl font-bold">إطلالة اليوم</h1><p className="mt-2 text-gray-500">صوّتي لأجمل إطلالة!</p></div>
      {isLoading ? <CardSkeleton /> : today ? (
        <Card padding="lg" className="mb-8 border-2 border-amber-300 dark:border-amber-700 bg-gradient-to-br from-amber-50 to-yellow-50 dark:from-amber-950 dark:to-yellow-950 text-center">
          <span className="text-5xl">🌟</span><h2 className="mt-2 text-xl font-bold">إطلالة اليوم</h2><p className="text-lg font-bold mt-1">{today.title as string}</p><p className="text-sm text-gray-500">👩‍🎨 {today.technicianName as string} · ❤️ {today.votes as number} صوت</p>
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
        {looks.map((l: Record<string,unknown>) => (
          <Card key={l.id as number} padding="md" className="text-center">
            ...
            <h3 className="font-bold mt-2">{l.title as string}</h3><p className="text-xs text-gray-500">{l.userName as string} · 👩‍🎨 {l.technicianName as string}</p>
