...
export default function BeautyExpoPage(): JSX.Element {
  ...
  return (
    <div className="mx-auto max-w-5xl px-4 py-12">
      <div className="mb-8 text-center"><span className="text-6xl">🎪</span><h1 className="mt-4 text-3xl font-bold">معرض التجميل الافتراضي</h1><p className="mt-2 text-gray-500">تجولي في أجنحة أشهر الماركات العالمية</p></div>
      {isLoading ? <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">{Array.from({length:5},(_,i)=><CardSkeleton key={i}/>)}</div> :
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">{booths.map((b: Record<string,unknown>) => (
          <Card key={b.id as number} padding="lg" className="text-center hover:shadow-xl transition-all">
            ...
            <p className="text-sm text-gray-500 mt-1">{b.description as string}</p>
            ...
            <p className="text-xs text-gray-400 mt-3">👥 {b.visitors as number} زائر</p>
          </Card>
        ))}</div>
      }
    </div>
