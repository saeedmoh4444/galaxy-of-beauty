...
export default function ServiceMatchmakerPage(): JSX.Element {
  ...
  const { data: questions } = api.serviceMatchmaker.questions.useQuery() as { data: Array<Record<string,unknown>> | undefined };
      ...
      {searchAnswers ? (
        ...
        <div className="space-y-3">
          {services.map((s: Record<string,unknown>) => (
            <Card key={s.id as number} padding="lg" className="flex items-center gap-4 hover:shadow-lg transition-all">
              <span className="text-4xl">{s.emoji as string}</span>
              <div className="flex-1"><h3 className="font-bold text-lg">{s.nameAr as string}</h3><span className="rounded-full bg-brand-100 dark:bg-brand-900 px-2 py-0.5 text-xs font-bold text-brand-700">{s.score as number}% تطابق</span></div>
              <div className="text-right"><p className="text-xl font-extrabold text-brand-600">{formatCurrency(s.price as number)} ر.س</p><Link href="/bookings/create"><Button size="sm" className="mt-2">احجزي ←</Button></Link></div>
            </Card>
          ))}
