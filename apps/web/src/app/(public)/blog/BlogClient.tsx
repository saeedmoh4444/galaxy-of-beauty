...
interface BlogPost {
  id: number; titleJson: Record<string, string>; bodyJson: Record<string, string>;
  slug: string; imageUrl: string | null; tags: string[]; publishedAt: string | null;
}
...
export function BlogClient({ initialPosts, initialTotal }: { initialPosts: unknown[]; initialTotal: number }): JSX.Element {
  ...
  const posts: BlogPost[] = useMemo(() => data?.items ?? (isLoading ? (initialPosts as BlogPost[]) : []), [data?.items, isLoading]);
  const totalPages = data ? Math.ceil(data.total / POSTS_PER_PAGE) : Math.ceil(initialTotal / POSTS_PER_PAGE) || 1;
  ...
  return (
    <div className="mx-auto max-w-6xl px-4 py-12">
      <div className="mb-10 text-center">
        ...
        <p className="mt-2 text-text-secondary dark:text-gray-400">نصائح، اتجاهات، وأسرار العناية بالجمال</p>
      </div>
      ...
      <div className="mb-8 flex flex-wrap justify-center gap-2">
        ...
        {ALL_TAGS.slice(0, 8).map((tag) => (
          <button key={tag} onClick={() => { setActiveTag(tag === activeTag ? null : tag); setPage(1); }}
      ...
      {isLoading && !initialPosts.length ? (
        <div className="grid gap-8 sm:grid-cols-2 lg:grid-cols-3">{Array.from({ length: 6 }, (_, i) => <CardSkeleton key={i} />)}</div>
      ) : isError ? (
        <ErrorAlert message="فشل تحميل المقالات" onRetry={() => refetch()} />
      ) : posts.length === 0 ? (
        <EmptyState title="لا توجد مقالات بعد" description="لم ننشر مقالات بعد. تابعي المدونة قريباً! 📝" />
      ) : filteredPosts.length === 0 ? (
        <EmptyState title={`لا توجد مقالات في "${activeTag}"`} description="جربي تصفية بوسم آخر" action={{ label: 'عرض الكل', onPress: () => setActiveTag(null) }} />
      ) : (
        <>
                ...
                <Link key={post.id} href={`/blog/${post.slug}`} className="group">
                  <article className="overflow-hidden rounded-2xl border border-edge bg-white transition-all hover:shadow-xl hover:-translate-y-1 dark:border-gray-800 dark:bg-gray-900">
                    <div className="relative flex h-48 items-center justify-center bg-gradient-to-br from-brand-100 to-accent-100 text-5xl dark:from-brand-900 dark:to-accent-900">
                      {post.imageUrl ? <img src={post.imageUrl} alt={title} className="h-full w-full object-cover transition-transform group-hover:scale-105" loading="lazy" /> : <span>✨</span>}
                    </div>
                    <div className="p-5">
                      <h2 className="text-lg font-bold text-text-primary group-hover:text-brand-600 dark:text-gray-100 line-clamp-2">{title}</h2>
          ...
          {totalPages > 1 && <div className="mt-10 flex justify-center"><Pagination page={page} totalPages={totalPages} onPageChange={setPage} /></div>}
        </>
      )}
    </div>
  );
