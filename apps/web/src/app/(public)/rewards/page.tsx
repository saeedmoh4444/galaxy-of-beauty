    ...
    <div className="mx-auto max-w-5xl px-4 py-12">
      ...
      <div className="grid gap-6 sm:grid-cols-3 mb-12">
        {Object.entries(TIER_LABELS).map(([key, t]) => (
          <Card key={key} padding="lg" className={`bg-gradient-to-br ${t.color} text-white text-center`}>
            <span className="text-4xl">{t.emoji}</span>
            <h3 className="mt-2 text-xl font-bold">{t.name}</h3>
            <p className="text-sm opacity-80">{key === 'SILVER' ? 'تبدأ من ٠ نقطة' : key === 'GOLD' ? 'من ٥٠٠ نقطة' : 'من ٢٠٠٠ نقطة'}</p>
          </Card>
        ))}
      </div>
          ...
          {rewards.map((r: Record<string, any>) => {
            ...
            return (
              <Card key={r.id} padding="lg" className="relative">
                <div className="absolute top-3 left-3 rounded-full bg-brand-100 px-3 py-1 text-xs font-bold text-brand-700">{r.pointsCost} نقطة</div>
                <div className="text-center pt-4">
                  ...
                  {r.minTier && r.minTier !== 'SILVER' && (
                    <p className="mt-2 text-xs text-amber-600">للعضوية {TIER_LABELS[r.minTier]?.name} فأعلى</p>
                  )}
                </div>
              </Card>
            );
