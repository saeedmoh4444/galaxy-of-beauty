...
export default function ProductScannerScreen(): JSX.Element {
  ...
  const result = (trpc as any).productScanner?.scan?.useQuery?.({ barcode }, { enabled: scanned && barcode.length > 0 }) ?? { data: null, isLoading: false, isError: false, refetch: () => {} };

  return (
    <ScreenState isLoading={scanned && result.isLoading} isError={scanned && result.isError} isEmpty={false} errorMessage="لم يتم العثور على المنتج" onRetry={() => result.refetch()}>
      ...
      {(result.data as any) ? (
        <View style={styles.result}><Text style={styles.productName}>{(result.data as any)?.nameJson?.ar ?? (result.data as any)?.name ?? ''}</Text></View>
      ) : null}
    </ScreenState>
  );
}
...
const styles = StyleSheet.create({
  ...
  scanBtn: { backgroundColor: COLORS.brand, borderRadius: 16, padding: 24, alignItems: 'center', marginBottom: 20 },
