    ...
    return (
      <ScrollView style={styles.c} contentContainerStyle={styles.i}>
        ...
        <View style={styles.grid}>
          {SKIN_TYPES.map((t) => (
            <TouchableOpacity key={t.key} onPress={() => setSkinType(t.key)} style={[styles.skinBtn, skinType === t.key && styles.skinBtnActive]}>
              <Text style={styles.skinEmoji}>{t.emoji}</Text>
              <Text style={[styles.skinLabel, skinType === t.key && styles.skinLabelActive]}>{t.label}</Text>
            </TouchableOpacity>
          ))}
        </View>
        <TouchableOpacity onPress={generate} style={styles.genBtn}><Text style={styles.genBtnText}>🧠 توليد الروتين</Text></TouchableOpacity>
      </ScrollView>
    );
  }
    ...
    <ScrollView style={styles.c} contentContainerStyle={styles.i}>
      ...
      <View style={styles.section}>
        ...
        {morning.map((s: any, i: number) => (
          <View key={i} style={styles.step}><Text style={styles.stepEmoji}>{s.emoji}</Text><View><Text style={styles.stepLabel}>{s.stepAr}</Text><Text style={styles.stepDur}>{s.duration}</Text></View></View>
        ))}
      </View>
      <View style={styles.section}>
      ...
      {tips.length > 0 && (
        <View style={styles.section}>
          ...
          {tips.map((tip: string, i: number) => <Text key={i} style={styles.tip}>• {tip}</Text>)}
        </View>
      )}
      <TouchableOpacity onPress={() => { setGenerated(false); setData(null); }} style={styles.resetBtn}><Text style={styles.resetBtnText}>🔄 إعادة</Text></TouchableOpacity>
    </ScrollView>
  );
}
...
const styles = StyleSheet.create({
  ...
  skinBtn: { width: '45%', backgroundColor: '#fff', borderRadius: 16, padding: 16, alignItems: 'center', borderWidth: 2, borderColor: '#e5e7eb' },
  skinBtnActive: { borderColor: '#7c3aed', backgroundColor: '#faf5ff' },
  skinEmoji: { fontSize: 36 }, skinLabel: { fontSize: 14, fontWeight: '600', color: '#6b7280', marginTop: 6 },
  ...
  step: { flexDirection: 'row', alignItems: 'center', gap: 12, paddingVertical: 8, borderBottomWidth: 1, borderBottomColor: '#f3f4f6' },
  stepEmoji: { fontSize: 28 }, stepLabel: { fontSize: 14, fontWeight: '600', color: '#111827' }, stepDur: { fontSize: 12, color: '#6b7280' },
  tip: { fontSize: 13, color: '#6b7280', paddingVertical: 4, paddingHorizontal: 4 },
  resetBtn: { backgroundColor: '#f3f4f6', borderRadius: 14, padding: 14, alignItems: 'center', marginTop: 8 },
