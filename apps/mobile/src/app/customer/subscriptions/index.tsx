...
export default function SubscriptionsScreen(): JSX.Element {
  const sub = (trpc.subscriptions as any).getMySubscription?.useQuery?.({}) ?? { data: null, isLoading: false, isError: false, refetch: () => {} };
    ...
    >
      ...
      {subscription ? (
        <View style={styles.activeCard}>
          ...
          <Text style={styles.status}>{subscription.status as string === 'ACTIVE' ? '🟢 نشط' : '⏸️ متوقف'}</Text>
          <Text style={styles.date}>ينتهي: {new Date(subscription.currentPeriodEnd as string).toLocaleDateString('ar-SA')}</Text>
        </View>
      ) : (
        <View style={styles.activeCard}>
...
const styles = StyleSheet.create({
  ...
  activeCard: { backgroundColor: COLORS.brand, borderRadius: 16, padding: 20, marginBottom: 24, alignItems: 'center' },
  ...
  planCard: { backgroundColor: COLORS.white, borderRadius: 14, padding: 16, marginBottom: 8, alignItems: 'center', shadowColor: '#000', shadowOpacity: 0.05, shadowRadius: 4, elevation: 2 },
  ...
  subscribeBtn: { marginTop: 12, backgroundColor: COLORS.brand, borderRadius: 10, paddingHorizontal: 20, paddingVertical: 10 },
