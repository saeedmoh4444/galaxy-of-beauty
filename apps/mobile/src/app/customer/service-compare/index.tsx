  ...
  return (
    <ScrollView style={styles.c} contentContainerStyle={styles.i} refreshControl={<RefreshControl refreshing={refreshing} onRefresh={() => fetch(true)} colors={['#0891b2']} />}>
      ...
      <View style={styles.grid}>
        {services.slice(0, 12).map((s: any) => { const isSel = selected.includes(s.id); return (
          <TouchableOpacity key={s.id} onPress={() => toggle(s.id)} style={[styles.ch, isSel && styles.cha]}><Text style={styles.ce}>{s.emoji as string ?? '💆‍♀️'}</Text><Text style={[styles.cn, isSel && styles.cna]}>{(s.titleJson as any)?.ar as string ?? s.nameAr as string}</Text><Text style={styles.cp}>{(s.basePrice as number)?.toLocaleString()} ر.س</Text></TouchableOpacity>);})}
      </View>
      {compareItems.length >= 2 && <View style={styles.tbl}><Text style={styles.ttl}>📊 المقارنة</Text>
        {compareItems.map((s: any) => (<View key={s.id} style={styles.cc}><Text style={styles.ct}>{(s.titleJson as any)?.ar as string}</Text><View style={styles.cr}><Text style={styles.cl}>💰</Text><Text style={styles.cv}>{(s.basePrice as number)?.toLocaleString()} ر.س</Text></View><View style={styles.cr}><Text style={styles.cl}>⏱️</Text><Text style={styles.cv}>{s.durationMin as number} دقيقة</Text></View></View>))}</View>}
    </ScrollView>
  );
}
const styles = StyleSheet.create({
  ...
  cha: { borderColor: '#0891b2', backgroundColor: '#ecfeff' }, ce: { fontSize: 24 }, cn: { fontSize: 10, fontWeight: '600', color: '#6b7280', marginTop: 4, textAlign: 'center' }, cna: { color: '#0891b2' }, cp: { fontSize: 10, color: '#9ca3af', marginTop: 2 },
  tbl: { gap: 10 }, ttl: { fontSize: 16, fontWeight: '700', color: '#111827', marginBottom: 8 },
  cc: { backgroundColor: '#fff', borderRadius: 14, padding: 14 }, ct: { fontSize: 15, fontWeight: '700', color: '#0891b2', marginBottom: 8 },
  cr: { flexDirection: 'row', justifyContent: 'space-between', paddingVertical: 4 }, cl: { fontSize: 13, color: '#6b7280' }, cv: { fontSize: 13, fontWeight: '600', color: '#111827' },
});
