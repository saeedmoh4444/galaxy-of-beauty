...
export default function VirtualConsultationScreen(): JSX.Element {
  const { data: bookings, loading, error, refetch, refreshing, refresh } = useQuery(() => (trpc as any).virtualConsultation.myConsultations.query());
  ...
  const handleBook = async () => {
    ...
    try { await (trpc as any).virtualConsultation.book.mutate({ consultantType: consultant.key, scheduledAt: new Date().toISOString(), slot, price: consultant.price }); setBooked(true); } catch {}
  };
  ...
  return (
    <ScrollView style={s.c} contentContainerStyle={s.i} refreshControl={<RefreshControl refreshing={refreshing} onRefresh={refresh} colors={['#db2777']} />}>
      ...
      <View style={s.grid}>
        {CONSULTANTS.map(c => (
          <TouchableOpacity key={c.key} onPress={() => { setSelected(c.key); setSlot(null); }} style={[s.consCard, selected===c.key&&{borderColor:'#db2777',backgroundColor:'#fdf2f8'}]}>
            ...
            <Text style={{fontWeight:'700',color:'#db2777',textAlign:'center',marginTop:4}}>{c.price} ر.س · ⭐{c.rating}</Text>
          </TouchableOpacity>
        ))}
      </View>
      ...
      {myBookings.length > 0 && <View style={{marginTop:20}}>
        <Text style={{fontSize:16,fontWeight:'700',color:'#111827',marginBottom:8}}>📋 حجوزاتي</Text>
        {myBookings.map((b:any,i:number)=>(<View key={i} style={{backgroundColor:'#fff',borderRadius:10,padding:12,marginBottom:6,flexDirection:'row',justifyContent:'space-between'}}><Text style={{fontSize:13}}>{b.consultantType} — {b.slot}</Text><Text style={{fontSize:11,color:b.status==='CONFIRMED'?'#059669':'#f59e0b'}}>{b.status}</Text></View>))}
      </View>}
    </ScrollView>
  );
}
...
const s = StyleSheet.create({
  ...
  consCard:{width:'48%',backgroundColor:'#fff',borderRadius:14,borderWidth:2,borderColor:'#e5e7eb',padding:12,marginBottom:8},
  slotBtn:{paddingHorizontal:16,paddingVertical:8,backgroundColor:'#f3f4f6',borderRadius:20},
