...
const TYPES: Record<string,string> = { workshop:'🎓 ورشة', masterclass:'👑 ماستر كلاس', launch:'🚀 إطلاق', seasonal:'🌸 موسمي' };
...
export default function BeautyEventsScreen(): JSX.Element {
  ...
  return (
    <ScrollView style={s.c} contentContainerStyle={s.i} refreshControl={<RefreshControl refreshing={refreshing} onRefresh={refresh} colors={['#db2777']} />}>
      ...
      {items.map((e:any)=>{const isReg=registeredIds.has(e.id);return(
        <View key={e.id} style={s.card}>
          <View style={{alignItems:'center'}}><Text style={{fontSize:36}}>{TYPES[e.eventType]?.split(' ')[0]??'🎪'}</Text><Text style={{fontWeight:'800',fontSize:16,marginTop:8}}>{e.nameJson?.ar}</Text><Text style={{fontSize:12,color:'#6b7280',marginTop:2}}>{TYPES[e.eventType]}</Text><Text style={{fontSize:12,color:'#6b7280'}}>{e.location} · {new Date(e.startsAt).toLocaleDateString('ar-SA')}</Text><Text style={{fontWeight:'800',fontSize:20,color:'#db2777',marginTop:6}}>{e.price?`${e.price} ر.س`:'مجانية'}</Text></View>
          <TouchableOpacity onPress={()=>isReg?handleCancel(e.id):handleRegister(e.id)} style={[s.btn,isReg&&{backgroundColor:'#e5e7eb'}]}><Text style={[s.btnText,isReg&&{color:'#374151'}]}>{isReg?'✅ مسجلة — إلغاء':'📝 سجلي الآن'}</Text></TouchableOpacity>
        </View>
      )})}
    </ScrollView>
  );
...
const s=StyleSheet.create({c:{flex:1,backgroundColor:'#fdf2f8'},i:{padding:16,paddingBottom:40},t:{fontSize:24,fontWeight:'800',color:'#111827',textAlign:'center',marginBottom:8},sub:{fontSize:14,color:'#6b7280',textAlign:'center',marginBottom:20},card:{backgroundColor:'#fff',borderRadius:16,padding:18,marginBottom:12},btn:{backgroundColor:'#db2777',borderRadius:10,paddingVertical:12,alignItems:'center',marginTop:12},btnText:{color:'#fff',fontSize:14,fontWeight:'600'}});
