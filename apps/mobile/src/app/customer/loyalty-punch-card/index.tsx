...
export default function LoyaltyPunchCardScreen(): JSX.Element {
  ...
  const fetch = useCallback((isRefresh = false) => {
    ...
    ((trpc as any).loyaltyPunchCard.status.query() as any).then((d: any) => { setData(d); setLoading(false); setRefreshing(false); }).catch(() => { setLoading(false); setRefreshing(false); });
  }, []);
  ...
  return (
    <ScrollView style={styles.c} contentContainerStyle={styles.i} refreshControl={<RefreshControl refreshing={refreshing} onRefresh={() => fetch(true)} colors={['#f59e0b']} />}>
      ...
      <View style={styles.card}>
        <Text style={styles.count}>{punches}/{total}</Text>
        <View style={styles.grid}>
          {Array.from({length: total}, (_, i) => (
            <View key={i} style={[styles.punch, i < punches && styles.punched]}><Text style={styles.punchText}>{i < punches ? '💎' : '○'}</Text></View>
          ))}
        </View>
      </View>
    </ScrollView>
...
const styles = StyleSheet.create({
  ...
  card: { backgroundColor: '#fff', borderRadius: 20, padding: 24, alignItems: 'center', width: '100%' },
  count: { fontSize: 28, fontWeight: '800', color: '#d97706', marginBottom: 16 },
  grid: { flexDirection: 'row', flexWrap: 'wrap', gap: 8, justifyContent: 'center' },
  punch: { width: 44, height: 44, borderRadius: 22, backgroundColor: '#f3f4f6', alignItems: 'center', justifyContent: 'center' },
