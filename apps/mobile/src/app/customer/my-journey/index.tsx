...
export default function MyJourneyScreen(): JSX.Element {
  const journey = (trpc as any).beautyDashboard?.getJourney?.useQuery?.() ?? {
    data: null,
    isLoading: false,
    isError: false,
    refetch: () => {},
        ...
        <View key={i} style={styles.card}>
          ...
          <View style={styles.content}>
            ...
            <Text style={styles.date}>
              {item.date ? new Date(item.date as string).toLocaleDateString('ar-SA') : ''}
            </Text>
          </View>
        </View>
      ))}
