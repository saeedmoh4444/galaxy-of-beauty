...
export default function PaymentsScreen(): JSX.Element {
  const payments = (trpc as any).payments?.list?.useQuery?.({ limit: EXTENDED_PAGE_SIZE }) ?? { data: null, isLoading: false, isError: false, refetch: () => {} };
  const data = payments.data as unknown[] | undefined;

  return (
    <ScreenState isLoading={payments.isLoading} isError={payments.isError} isEmpty={!data || data.length === 0} errorMessage="فشل تحميل المدفوعات" emptyTitle="لا توجد مدفوعات" onRetry={() => payments.refetch()}>
        ...
        renderItem={({ item }: { item: Record<string, unknown> }) => (
          <View style={styles.row}>
            <View>
              ...
              <Text style={styles.date}>{new Date(item.createdAt as string).toLocaleDateString('ar-SA')}</Text>
            </View>
            <Text style={[styles.amount, { color: item.status === 'CAPTURED' ? COLORS.success : COLORS.danger }]}>
