...
const COLORS = { brand: '#7c3aed', white: '#ffffff', gray50: '#faf5ff', gray400: '#6b7280', gray900: '#111827' };
  ...
  return (
    ...
    >
      ...
      <View style={styles.statsRow}>
        <View style={styles.statCard}><Text style={styles.statNum}>{String(data?.bookingCount ?? 0)}</Text><Text style={styles.statLabel}>حجوزات</Text></View>
        <View style={styles.statCard}><Text style={styles.statNum}>{String(data?.totalSpent ?? 0)} ر.س</Text><Text style={styles.statLabel}>الإنفاق</Text></View>
        <View style={styles.statCard}><Text style={styles.statNum}>🔥 {String((data?.streakInfo as any)?.currentStreak ?? 0)}</Text><Text style={styles.statLabel}>الاستمرارية</Text></View>
      </View>
      <View style={styles.quickLinks}>
        {[
          ...
          <TouchableOpacity key={i} style={[styles.qlBtn, { backgroundColor: ql.c }]} onPress={() => router.push(ql.h as any)}>
