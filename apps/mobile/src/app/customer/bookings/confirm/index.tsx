    ...
    <ScrollView style={styles.c} contentContainerStyle={styles.i}>
      ...
      <View style={styles.card}>
        ...
        <View style={styles.row}>
          ...
          <Text style={styles.value}>{new Date(bookingDate).toLocaleDateString('ar-SA', { weekday: 'long', year: 'numeric', month: 'long', day: 'numeric' })}</Text>
        </View>
        <View style={styles.row}>
          <Text style={styles.label}>الوقت</Text>
          <Text style={styles.value}>{new Date(bookingDate).toLocaleTimeString('ar-SA', { hour: '2-digit', minute: '2-digit' })}</Text>
        </View>
      </View>

      <TouchableOpacity style={styles.viewBtn}><Text style={styles.viewBtnText}>📅 عرض حجوزاتي</Text></TouchableOpacity>
      <TouchableOpacity style={styles.bookBtn}><Text style={styles.bookBtnText}>💆‍♀️ احجزي خدمة أخرى</Text></TouchableOpacity>
    </ScrollView>
  );
}

const styles = StyleSheet.create({
  c: { flex: 1, backgroundColor: '#f0fdf4' }, i: { padding: 30, paddingTop: 60, alignItems: 'center', paddingBottom: 60 },
  iconCircle: { width: 100, height: 100, borderRadius: 50, backgroundColor: '#dcfce7', alignItems: 'center', justifyContent: 'center', marginBottom: 20 },
  ...
  viewBtn: { backgroundColor: '#f3f4f6', borderRadius: 14, padding: 16, alignItems: 'center', width: '100%', marginBottom: 10 },
  viewBtnText: { fontSize: 14, fontWeight: '600', color: '#6b7280' },
  bookBtn: { backgroundColor: '#059669', borderRadius: 14, padding: 16, alignItems: 'center', width: '100%' },
