...
export default function BookingDetailScreen(): JSX.Element {
  ...
  const detail = (trpc as any).bookings?.getById?.useQuery?.({ id: Number(id) }, { enabled: !!id }) ?? { data: null, isLoading: false, isError: false, refetch: () => {} };
  const data = detail.data as Record<string, unknown> | undefined;

  return (
    <ScreenState isLoading={detail.isLoading} isError={detail.isError} isEmpty={!data} errorMessage="فشل تحميل الحجز" onRetry={() => detail.refetch()}>
      <ScrollView style={styles.c} contentContainerStyle={styles.i}>
        <Text style={styles.title}>📅 تفاصيل الحجز</Text>
        {(data ? [
          { label: 'كود الحجز', value: data.bookingCode as string },
          { label: 'الحالة', value: SL[data.status as string] ?? (data.status as string), color: SC[data.status as string]?.color },
