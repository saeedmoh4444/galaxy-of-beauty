...
export default function ServiceHistoryScreen(): JSX.Element {
  ...
  return (
    <ScreenState isLoading={bookings.isLoading} isError={bookings.isError} isEmpty={!data || data.length === 0} errorMessage="فشل تحميل السجل" emptyTitle="لا يوجد سجل خدمات" emptyDescription="ستظهر خدماتكِ السابقة هنا" onRetry={() => bookings.refetch()}>
        ...
        renderItem={({ item }: { item: Record<string, unknown> }) => (
          <View style={styles.row}>
            <View>
              ...
              <Text style={styles.date}>{new Date(item.startAt as string).toLocaleDateString('ar-SA')}</Text>
            </View>
