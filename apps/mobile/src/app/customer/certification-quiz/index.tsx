...
export default function CertificationQuizScreen(): JSX.Element {
  ...
  const fetch = useCallback((isRefresh = false) => {
    ...
    ((trpc as any).certificationQuiz.list.query() as any).then((d: any) => { setData(d || []); setLoading(false); setRefreshing(false); }).catch(() => { setLoading(false); setRefreshing(false); });
  }, []);
    ...
    <ScrollView style={styles.c} contentContainerStyle={styles.i} refreshControl={<RefreshControl refreshing={refreshing} onRefresh={() => fetch(true)} colors={['#0891b2']} />}>
      ...
      {data.map((q: any, i: number) => (
        <View key={i} style={styles.card}>
          ...
          <TouchableOpacity style={styles.startBtn}><Text style={styles.startText}>بدء</Text></TouchableOpacity>
        </View>
      ))}
    </ScrollView>
  );
...
const styles = StyleSheet.create({
  ...
  qTitle: { fontSize: 15, fontWeight: '700', color: '#111827' }, qDesc: { fontSize: 13, color: '#6b7280', marginTop: 4 },
  startBtn: { alignSelf: 'flex-end', backgroundColor: '#0891b2', borderRadius: 10, paddingHorizontal: 20, paddingVertical: 8, marginTop: 8 },
