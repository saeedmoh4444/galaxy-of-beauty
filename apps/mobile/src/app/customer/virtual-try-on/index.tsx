...
export default function VirtualTryOnScreen(): JSX.Element {
  ...
  const fetch = useCallback((isRefresh = false) => {
    ...
    ((trpc as any).virtualTryOn.products.query() as any).then((d: any) => { setData(d || []); setLoading(false); setRefreshing(false); }).catch(() => { setLoading(false); setRefreshing(false); });
  }, []);
    ...
    <ScrollView style={styles.c} contentContainerStyle={styles.i} refreshControl={<RefreshControl refreshing={refreshing} onRefresh={() => fetch(true)} colors={['#ec4899']} />}>
      ...
      {data.map((p: any, i: number) => (
        <TouchableOpacity key={i} style={styles.card}>
          <View style={[styles.swatch,{backgroundColor:p.hex as string}]} /><Text style={styles.name}>{p.nameAr as string}</Text>
        </TouchableOpacity>
      ))}
    </ScrollView>
  );
...
const styles = StyleSheet.create({
  ...
  swatch: { width: 40, height: 40, borderRadius: 20 }, name: { fontSize: 14, fontWeight: '600', color: '#111827' },
});
