...
export default function HairColorSimScreen(): JSX.Element {
  ...
  const fetch = useCallback((isRefresh = false) => {
    ...
    ((trpc as any).hairColorSim.colors.query() as any).then((d: any) => { setColors(d || []); setLoading(false); setRefreshing(false); }).catch(() => { setLoading(false); setRefreshing(false); });
  }, []);
...
const styles = StyleSheet.create({
  ...
  color: { width: '30%', aspectRatio: 1, borderRadius: 16, alignItems: 'center', justifyContent: 'flex-end', padding: 10 },
  colorName: { fontSize: 11, fontWeight: '600', color: '#fff', backgroundColor: 'rgba(0,0,0,0.4)', borderRadius: 6, paddingHorizontal: 6, paddingVertical: 2 },
});
