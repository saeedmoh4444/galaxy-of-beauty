...
export default function WaitlistScreen(): JSX.Element {
  ...
  const fetch = useCallback((isRefresh = false) => {
    ...
    ((trpc as any).waitlist.myEntries.query() as any).then((d: any) => { setData(d || []); setLoading(false); setRefreshing(false); }).catch(() => { setLoading(false); setRefreshing(false); });
  }, []);
  ...
  return (
    <ScrollView style={styles.c} contentContainerStyle={styles.i} refreshControl={<RefreshControl refreshing={refreshing} onRefresh={() => fetch(true)} colors={['#f59e0b']} />}>
      ...
      {data.map((w: any, i: number) => (
        <View key={i} style={styles.card}><Text style={styles.emoji}>⏳</Text><View style={{flex:1}}><Text style={styles.name}>{w.serviceName as string}</Text><Text style={styles.pos}>الموقع: #{w.position as number}</Text></View></View>
      ))}
    </ScrollView>
  );
}
const styles = StyleSheet.create({
  ...
  emoji: { fontSize: 28 }, name: { fontSize: 14, fontWeight: '600', color: '#111827' }, pos: { fontSize: 12, color: '#6b7280', marginTop: 2 },
});
