    ...
    <ScrollView style={styles.c} contentContainerStyle={styles.i} refreshControl={<RefreshControl refreshing={refreshing} onRefresh={() => fetch(true)} colors={['#6366f1']} />}>
      ...
      <View style={styles.grid}>
        {services.map((s: any) => {
          ...
          return (
            <TouchableOpacity key={s.id} onPress={() => toggle(s.id)} style={[styles.chip, isSel && styles.chipActive]}>
              <Text style={styles.chipEmoji}>{s.emoji as string ?? '💆‍♀️'}</Text>
              <Text style={[styles.chipName, isSel && styles.chipNameActive]}>{s.nameAr as string}</Text>
        ...
        <View style={styles.table}>
          ...
          {compareItems.map((s: any) => (
            <View key={s.id} style={styles.compareCard}>
              ...
              <View style={styles.cRow}><Text style={styles.cLabel}>💰</Text><Text style={styles.cVal}>{(s.price as number)?.toLocaleString()} ر.س</Text></View>
              <View style={styles.cRow}><Text style={styles.cLabel}>⏱️</Text><Text style={styles.cVal}>{s.duration as string}</Text></View>
            </View>
          ))}
        </View>
      )}
...
const styles = StyleSheet.create({
  ...
  chipNameActive: { color: '#4f46e5' }, chipPrice: { fontSize: 10, color: '#9ca3af', marginTop: 2 },
  table: { gap: 10 }, tableTitle: { fontSize: 16, fontWeight: '700', color: '#111827', marginBottom: 8 },
  compareCard: { backgroundColor: '#fff', borderRadius: 14, padding: 14 }, cTitle: { fontSize: 15, fontWeight: '700', color: '#4f46e5', marginBottom: 8 },
  cRow: { flexDirection: 'row', justifyContent: 'space-between', paddingVertical: 4 }, cLabel: { fontSize: 13, color: '#6b7280' }, cVal: { fontSize: 13, fontWeight: '600', color: '#111827' },
});
