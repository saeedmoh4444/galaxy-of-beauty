...
export default function FlashDealsScreen(): JSX.Element {
  const { data: deals, loading, error, refreshing, refetch, refresh } = useQuery(() => (trpc as any).flashDeals.active.query());
  ...
  return (
    <ScrollView style={styles.c} contentContainerStyle={styles.i} refreshControl={<RefreshControl refreshing={refreshing} onRefresh={refresh} colors={['#dc2626']} />}>
      ...
      {items.length === 0 ? <Text style={styles.e}>لا توجد عروض حالية</Text> :
          ...
          <View key={i} style={styles.card}>
            ...
            <View style={{flex:1}}>
              ...
              <View style={styles.priceRow}>
                <Text style={styles.oldPrice}>{(d.originalPrice as number)?.toLocaleString()} ر.س</Text>
                <Text style={styles.newPrice}>{(d.price as number)?.toLocaleString()} ر.س</Text>
              </View>
            </View>
            <View style={styles.discountBadge}><Text style={styles.discountText}>-{d.discount as number}%</Text></View>
          </View>
        ))
      }
    </ScrollView>
...
const styles = StyleSheet.create({
  ...
  dealEmoji: { fontSize: 32 }, dealName: { fontSize: 14, fontWeight: '600', color: '#111827' },
  ...
  discountBadge: { backgroundColor: '#fee2e2', borderRadius: 8, paddingHorizontal: 8, paddingVertical: 4 },
