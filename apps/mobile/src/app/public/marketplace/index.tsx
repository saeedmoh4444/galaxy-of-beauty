...
export default function MarketplaceScreen(): JSX.Element {
  ...
  return (
    <ScrollView style={styles.c} contentContainerStyle={styles.i} refreshControl={<RefreshControl refreshing={refreshing} onRefresh={() => fetch(true)} colors={['#db2777']} />}>
      ...
      <View style={styles.grid}>{products.length === 0 ? [{emoji:'🧴',title:'منتجات العناية',desc:'تصفحي المنتجات'},{emoji:'💄',title:'مستحضرات تجميل',desc:'أفضل الماركات'},{emoji:'💇‍♀️',title:'منتجات الشعر',desc:'عناية متكاملة'},{emoji:'💅',title:'منتجات الأظافر',desc:'ألوان رائعة'}].map((item,i) => (<View key={i} style={styles.card}><View style={styles.ci}><Text style={styles.ce}>{item.emoji}</Text></View><Text style={styles.ct}>{item.title}</Text><Text style={styles.cd}>{item.desc}</Text></View>)) : products.map((p: any) => (<TouchableOpacity key={p.id} style={styles.card}><View style={styles.ci}><Text style={styles.ce}>{p.emoji as string ?? '🧴'}</Text></View><Text style={styles.ct}>{p.nameAr as string ?? p.titleAr as string}</Text><Text style={styles.cp}>{(p.price as number)?.toLocaleString()} ر.س</Text></TouchableOpacity>))}</View>
    </ScrollView>
  );
}
const styles = StyleSheet.create({
  ...
  ci: { height: 120, borderRadius: 12, backgroundColor: '#fce7f3', alignItems: 'center', justifyContent: 'center', marginBottom: 8 },
  ce: { fontSize: 40 }, ct: { fontSize: 13, fontWeight: '700', color: '#111827', textAlign: 'right' }, cd: { fontSize: 11, color: '#6b7280', textAlign: 'right', marginTop: 2 }, cp: { fontSize: 14, fontWeight: '800', color: '#db2777', textAlign: 'right', marginTop: 4 },
});
