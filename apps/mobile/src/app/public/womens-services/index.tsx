...
export default function WomensServicesScreen(): JSX.Element {
  const { data: cats, loading, error, refreshing, refetch, refresh } = useQuery(() => (trpc as any).womensServices.categories.query());
  ...
  return (
    <ScrollView style={styles.c} contentContainerStyle={styles.i} refreshControl={<RefreshControl refreshing={refreshing} onRefresh={refresh} colors={['#be185d']} />}>
...
const styles = StyleSheet.create({
  ...
  svcCard: { flexDirection: 'row', justifyContent: 'space-between', paddingVertical: 8, borderBottomWidth: 1, borderBottomColor: '#f3f4f6' },
  svcName: { fontSize: 13, color: '#374151' }, svcPrice: { fontSize: 13, fontWeight: '600', color: '#be185d' },
});
