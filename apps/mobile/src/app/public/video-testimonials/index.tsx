...
export default function VideoTestimonialsScreen(): JSX.Element {
  ...
  const fetch = useCallback((isRefresh = false) => {
    ...
    ((trpc as any).videoTestimonials.list.query() as any).then((d: any) => { setVideos(d || []); setLoading(false); setRefreshing(false); }).catch(() => { setLoading(false); setRefreshing(false); });
  }, []);
  ...
  return (
    <ScrollView style={styles.c} contentContainerStyle={styles.i} refreshControl={<RefreshControl refreshing={refreshing} onRefresh={() => fetch(true)} colors={['#dc2626']} />}>
      ...
      {videos.map((v: any, i: number) => (<View key={i} style={styles.card}><Text style={styles.ve}>{v.emoji as string ?? '🎥'}</Text><View style={{flex:1}}><Text style={styles.vt}>{v.titleAr as string}</Text><Text style={styles.vm}>👩‍🎨 {v.technician as string} · 👁 {v.views as number}</Text></View></View>))}
    </ScrollView>
  );
}
const styles = StyleSheet.create({
  ...
  ve: { fontSize: 32 }, vt: { fontSize: 14, fontWeight: '600', color: '#111827' }, vm: { fontSize: 11, color: '#6b7280', marginTop: 2 },
});
