...
export default function ShopTheLookScreen(): JSX.Element {
  ...
  const fetch = useCallback((isRefresh = false) => {
    ...
    ((trpc as any).shopTheLook.looks.query() as any).then((d: any) => { setLooks(d || []); setLoading(false); setRefreshing(false); }).catch(() => { setLoading(false); setRefreshing(false); });
  }, []);
  ...
  return (
    <ScrollView style={styles.c} contentContainerStyle={styles.i} refreshControl={<RefreshControl refreshing={refreshing} onRefresh={() => fetch(true)} colors={['#ec4899']} />}>
      ...
      {looks.length === 0 ? <Text style={styles.e}>لا توجد إطلالات</Text> :
        looks.map((l: any) => (
          <View key={l.id} style={styles.card}>
            <View style={styles.lookHeader}>
              {l.imageUrl ? <Image source={{uri: l.imageUrl as string}} style={styles.lookImage} /> : <View style={styles.lookPlaceholder}><Text style={{fontSize:32}}>🛍️</Text></View>}
              <View style={{flex:1}}>
            ...
            {(l.products as any[])?.map((p: any) => (
              <View key={p.id} style={styles.product}>
                ...
                <View style={{alignItems:'flex-end'}}>
                  ...
                  <TouchableOpacity style={styles.buyBtn}><Text style={styles.buyBtnText}>شراء</Text></TouchableOpacity>
                </View>
              </View>
            ))}
          </View>
...
const styles = StyleSheet.create({
  ...
  lookImage: { width: 80, height: 80, borderRadius: 12 }, lookPlaceholder: { width: 80, height: 80, borderRadius: 12, backgroundColor: '#f3f4f6', alignItems: 'center', justifyContent: 'center' },
  lookTitle: { fontSize: 15, fontWeight: '600', color: '#111827' }, lookBy: { fontSize: 12, color: '#6b7280', marginTop: 4 },
  ...
  buyBtn: { backgroundColor: '#db2777', borderRadius: 8, paddingHorizontal: 12, paddingVertical: 6, marginTop: 2 },
