...
const QUESTIONS = [
  { key: 'occasion', q: 'ما هي المناسبة؟', options: ['🎂 عيد ميلاد', '👰 زفاف', '🎓 تخرج', '💐 شكر', '🎁 بدون مناسبة'] },
  { key: 'recipient', q: 'لمن الهدية؟', options: ['👯‍♀️ صديقة', '👩‍👧 أمي', '👭 أختي', '💑 زوجتي', '💝 نفسي'] },
...
export default function GiftQuizScreen(): JSX.Element {
  ...
  const recommendations = (trpc as any).giftQuiz?.getRecommendations?.useQuery?.(
    answers,
    { enabled: Object.keys(answers).length === QUESTIONS.length }
  ) ?? { data: null, isLoading: false, isError: false, refetch: () => {} };

  return (
    <ScreenState isLoading={recommendations.isLoading && Object.keys(answers).length === QUESTIONS.length} isError={recommendations.isError} isEmpty={false} errorMessage="فشل تحميل التوصيات" onRetry={() => recommendations.refetch()}>
      ...
      {step < QUESTIONS.length ? (
        ...
        <View>
          ...
          <TouchableOpacity style={styles.restartBtn} onPress={() => { setStep(0); setAnswers({}); }}>
...
const styles = StyleSheet.create({
  ...
  resultTitle: { fontSize: 18, fontWeight: '700', color: COLORS.gray900, marginBottom: 12, textAlign: 'center' },
  recCard: { flexDirection: 'row', alignItems: 'center', backgroundColor: COLORS.white, borderRadius: 14, padding: 14, marginBottom: 8 },
