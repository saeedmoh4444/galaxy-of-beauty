...
export default function BlogPostScreen(): JSX.Element {
  ...
  const fetch = useCallback((isRefresh = false) => {
    ...
    ((trpc as any).blog.getBySlug.query({ slug }) as any).then((d: any) => { setPost(d); setLoading(false); setRefreshing(false); }).catch(() => { setLoading(false); setRefreshing(false); });
  }, [slug]);
  ...
  return (
    <ScrollView style={styles.c} contentContainerStyle={styles.i} refreshControl={<RefreshControl refreshing={refreshing} onRefresh={() => fetch(true)} colors={['#7c3aed']} />}>
      ...
      <View style={styles.meta}>{post.publishedAt && <Text style={styles.date}>{new Date(post.publishedAt as string).toLocaleDateString('ar-SA',{year:'numeric',month:'long',day:'numeric'})}</Text>}
        {tags.length > 0 && <View style={styles.tags}>{tags.map((t: string) => <Text key={t} style={styles.tag}>{t}</Text>)}</View>}</View>
...
const styles = StyleSheet.create({
  ...
  meta: { marginBottom: 20 }, date: { fontSize: 13, color: '#9ca3af', textAlign: 'right', marginBottom: 8 },
