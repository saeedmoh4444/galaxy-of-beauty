...
export default function ApiDocsScreen(): JSX.Element {
  ...
  const fetch = useCallback((isRefresh = false) => {
    ...
    ((trpc as any).apiDocs.reference.query() as any).then((d: any) => { setData(d); setLoading(false); setRefreshing(false); }).catch(() => { setLoading(false); setRefreshing(false); });
  }, []);
  ...
  return (
    <ScrollView style={styles.c} contentContainerStyle={styles.i} refreshControl={<RefreshControl refreshing={refreshing} onRefresh={() => fetch(true)} colors={['#6366f1']} />}>
      ...
      {data && <View style={styles.card}><Text style={styles.ttl}>{data.title as string} v{data.version as string}</Text><Text style={styles.meta}>{data.endpoints as number} routers</Text></View>}
    </ScrollView>
  );
}
const styles = StyleSheet.create({
  ...
  ttl: { fontSize: 20, fontWeight: '700', color: '#111827' }, meta: { fontSize: 13, color: '#6b7280', marginTop: 4 },
});
