...
const questions = [
  ...
  { id: 'style', text: 'أسلوبك المفضل؟', options: [
        ...
        <View style={styles.resultCard}>
          ...
          <View style={styles.answers}>
            {Object.entries(answers).map(([qId, val]) => {
              ...
              return <Text key={qId} style={styles.answerRow}>{opt?.icon} {q?.text}: {opt?.label}</Text>;
            })}
          </View>
        </View>
        <TouchableOpacity onPress={() => { setStep(0); setAnswers({}); setDone(false); }} style={styles.btn}><Text style={styles.btnText}>🔄 إعادة</Text></TouchableOpacity>
      </ScrollView>
    );
  }
...
const styles = StyleSheet.create({
  ...
  options: { gap: 10 },
  option: { flexDirection: 'row', alignItems: 'center', gap: 12, backgroundColor: '#fff', borderRadius: 14, padding: 16, borderWidth: 1, borderColor: '#f3f4f6' },
  optionIcon: { fontSize: 28 }, optionLabel: { fontSize: 15, fontWeight: '600', color: '#111827' },
  resultCard: { backgroundColor: '#fff', borderRadius: 20, padding: 24, alignItems: 'center' },
  resultEmoji: { fontSize: 56 }, resultTitle: { fontSize: 20, fontWeight: '700', color: '#111827', marginTop: 12 },
  ...
  btn: { backgroundColor: '#db2777', borderRadius: 14, padding: 14, alignItems: 'center', marginTop: 16 },
