...
export default function BeautyExpoScreen(): JSX.Element {
  const expo = (trpc as any).beautyExpo?.list?.useQuery?.({}) ?? { data: null, isLoading: false, isError: false, refetch: () => {} };
  const data = expo.data as unknown[] | undefined;

  return (
    <ScreenState isLoading={expo.isLoading} isError={expo.isError} isEmpty={!data || data.length === 0} errorMessage="فشل تحميل المعرض" emptyTitle="لا توجد فعاليات" onRetry={() => expo.refetch()}>
      ...
      {(data as Record<string, unknown>[])?.map((e: Record<string, unknown>, i: number) => (
        <View key={i} style={styles.card}>
          ...
          <Text style={styles.date}>{e.startsAt ? new Date(e.startsAt as string).toLocaleDateString('ar-SA') : ''}</Text>
        </View>
      ))}
    </ScreenState>
  );
