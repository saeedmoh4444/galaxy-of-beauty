...
export default function MonitoringScreen(): JSX.Element {
  ...
  const fetch = useCallback((isRefresh = false) => {
    ...
    ((trpc as any).monitoring.health.query() as any).then((d: any) => { setHealth(d || {}); setLoading(false); setRefreshing(false); }).catch(() => { setLoading(false); setRefreshing(false); });
  }, []);
  ...
  return (
    <ScrollView style={styles.c} contentContainerStyle={styles.i} refreshControl={<RefreshControl refreshing={refreshing} onRefresh={() => fetch(true)} colors={['#059669']} />}>
      ...
      <View style={styles.grid}>
        {Object.entries(services).map(([key, svc]) => (
          <View key={key} style={[styles.svcCard,{borderColor:STATUS_COLORS[svc.status as string]??'#6b7280'}]}>
            <Text style={styles.svcEmoji}>{svc.status==='healthy'?'✅':'⚠️'}</Text><Text style={styles.svcKey}>{key}</Text><Text style={styles.svcPing}>{svc.ping as number}ms</Text>
          </View>
        ))}
      </View>
      <View style={styles.kpiRow}>
        ...
        <View style={styles.kpi}><Text style={styles.kpiEmoji}>👥</Text><Text style={[styles.kpiVal,{color:'#2563eb'}]}>{perf.activeSessions as number ?? 0}</Text><Text style={styles.kpiLabel}>جلسات</Text></View>
      </View>
    </ScrollView>
  );
}
