...
export default function AdminAnalyticsV2Screen(): JSX.Element {
  ...
  const fetch = useCallback((isRefresh = false) => {
    ...
    ((trpc as any).adminAnalyticsV2.dashboard.query() as any).then((d: any) => { setData(d || {}); setLoading(false); setRefreshing(false); }).catch(() => { setLoading(false); setRefreshing(false); });
  }, []);
  ...
  return (
    <ScrollView style={styles.c} contentContainerStyle={styles.i} refreshControl={<RefreshControl refreshing={refreshing} onRefresh={() => fetch(true)} colors={['#6366f1']} />}>
      ...
      {top.map((s: any, i: number) => (
        <View key={i} style={styles.row}><Text style={styles.rank}>#{i+1}</Text><Text style={styles.name}>{s.name as string}</Text><Text style={styles.stat}>{s.bookings as number} حجز</Text></View>
      ))}
    </ScrollView>
  );
}
...
const styles = StyleSheet.create({
  ...
  rank: { fontSize: 12, fontWeight: '700', color: '#4f46e5', width: 28 }, name: { flex: 1, fontSize: 13, fontWeight: '600', color: '#111827' },
