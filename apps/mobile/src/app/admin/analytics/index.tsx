  ...
  return (
    <ScrollView style={styles.c} contentContainerStyle={styles.i} refreshControl={<RefreshControl refreshing={refreshing} onRefresh={() => fetch(true)} colors={['#6366f1']} />}>
      ...
      <View style={styles.kpiRow}>
        <View style={styles.kpi}><Text style={styles.kpiEmoji}>👥</Text><Text style={styles.kpiVal}>{d.totalUsers as number ?? 0}</Text><Text style={styles.kpiLabel}>مستخدم</Text></View>
        <View style={styles.kpi}><Text style={styles.kpiEmoji}>📅</Text><Text style={[styles.kpiVal,{color:'#2563eb'}]}>{d.totalBookings as number ?? 0}</Text><Text style={styles.kpiLabel}>حجز</Text></View>
        <View style={styles.kpi}><Text style={styles.kpiEmoji}>💰</Text><Text style={[styles.kpiVal,{color:'#059669'}]}>{(d.totalRevenue as number ?? 0)?.toLocaleString()}</Text><Text style={styles.kpiLabel}>ر.س</Text></View>
      </View>
    </ScrollView>
  );
}
