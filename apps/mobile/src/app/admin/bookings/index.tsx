...
export default function AdminBookingsScreen(): JSX.Element {
  ...
  const fetch = useCallback((isRefresh = false) => {
    ...
    ((trpc as any).bookings.list.query({ page: 1, limit: 50 }) as any).then((d: any) => { setData(d?.bookings || []); setLoading(false); setRefreshing(false); }).catch(() => { setLoading(false); setRefreshing(false); });
  }, []);
