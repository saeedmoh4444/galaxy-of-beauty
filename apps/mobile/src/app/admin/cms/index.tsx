...
export default function AdminCMSScreen(): JSX.Element {
  ...
  const fetch = useCallback((isRefresh = false) => {
    ...
    ((trpc as any).cms.listCategories.query() as any).then((d: any) => { setData(d || []); setLoading(false); setRefreshing(false); }).catch(() => { setLoading(false); setRefreshing(false); });
  }, []);
    ...
    <ScrollView style={styles.c} contentContainerStyle={styles.i} refreshControl={<RefreshControl refreshing={refreshing} onRefresh={() => fetch(true)} colors={['#7c3aed']} />}>
      ...
      {data.map((cat: any, i: number) => (
        <View key={i} style={styles.card}>
          ...
          <View style={{flex:1}}><Text style={styles.name}>{(cat.nameJson as any)?.ar as string}</Text><Text style={styles.meta}>{cat.slug as string} · {cat._count?.services ?? 0} خدمات</Text></View>
        </View>
      ))}
    </ScrollView>
  );
