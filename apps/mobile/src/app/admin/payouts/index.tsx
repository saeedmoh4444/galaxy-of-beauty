...
export default function AdminPayoutsScreen(): JSX.Element {
  ...
  const fetch = useCallback((isRefresh = false) => {
    ...
    ((trpc as any).payouts.listForAdmin.query({}) as any).then((d: any) => { setData(d?.payouts || []); setLoading(false); setRefreshing(false); }).catch(() => { setLoading(false); setRefreshing(false); });
  }, []);

  useEffect(() => { fetch(); }, [fetch]);

  const process = (id: number) => { ((trpc as any).payouts.process.mutate({ payoutId: id }) as any).then(() => fetch()); };
      ...
      {data.map((p: any, i: number) => {
        ...
        return (
          <View key={i} style={styles.card}>
            <View style={{flex:1}}><Text style={styles.tech}>👩‍🎨 {p.technicianName as string}</Text><Text style={styles.amount}>{(p.amount as number)?.toLocaleString()} ر.س</Text></View>
            <View style={[styles.badge,{backgroundColor:s.bg}]}><Text style={[styles.badgeText,{color:s.color}]}>{s.label}</Text></View>
            {p.status === 'PENDING' && <TouchableOpacity onPress={() => process(p.id)} style={styles.btn}><Text style={styles.btnText}>معالجة</Text></TouchableOpacity>}
          </View>
        );
      })}
    </ScrollView>
...
const styles = StyleSheet.create({
  ...
  tech: { fontSize: 14, fontWeight: '600', color: '#111827' }, amount: { fontSize: 15, fontWeight: '700', color: '#0891b2', marginTop: 2 },
  badge: { borderRadius: 8, paddingHorizontal: 8, paddingVertical: 3 }, badgeText: { fontSize: 11, fontWeight: '700' },
