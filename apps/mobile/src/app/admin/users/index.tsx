...
export default function AdminUsersScreen(): JSX.Element {
  ...
  const fetch = useCallback((isRefresh = false) => {
    ...
    ((trpc as any).users.listAll.query({}) as any).then((d: any) => { setData(d?.items || []); setLoading(false); setRefreshing(false); }).catch(() => { setLoading(false); setRefreshing(false); });
  }, []);
    ...
    <ScrollView style={styles.c} contentContainerStyle={styles.i} refreshControl={<RefreshControl refreshing={refreshing} onRefresh={() => fetch(true)} colors={['#6366f1']} />}>
      ...
      {data.map((u: any, i: number) => (
        <View key={i} style={styles.card}>
          ...
          <View style={{flex:1}}><Text style={styles.name}>{u.name as string}</Text><Text style={styles.email}>{u.email as string}</Text></View>
...
const styles = StyleSheet.create({
  ...
  avatar: { fontSize: 28 }, name: { fontSize: 14, fontWeight: '600', color: '#111827' },
  email: { fontSize: 12, color: '#6b7280', marginTop: 2 }, role: { fontSize: 12, fontWeight: '600', color: '#4f46e5' },
});
