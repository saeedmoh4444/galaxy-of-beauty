...
export default function ServicesScreen(): JSX.Element {
  ...
  const services = trpc.services.list.useQuery({ sort: 'popular', limit: 20, search: search || undefined } as any);
  ...
  return (
    ...
    >
      ...
      <TextInput
        ...
        value={search}
        onChangeText={(t) => { setSearch(t); services.refetch(); }}
        ...
        <TouchableOpacity key={i} style={styles.card} activeOpacity={0.7}>
          <View style={styles.row}>
            <View style={styles.left}>
              <Text style={styles.name}>{((s.titleJson as Record<string,string>)?.ar) ?? (s.titleAr as string) ?? ''}</Text>
              <Text style={styles.desc} numberOfLines={2}>{((s.descriptionJson as Record<string,string>)?.ar)?.slice(0, 80) ?? ''}</Text>
            </View>
...
const styles = StyleSheet.create({
  ...
  search: { backgroundColor: COLORS.white, borderRadius: 12, padding: 12, marginBottom: 12, fontSize: 14, color: COLORS.gray900, borderWidth: 1, borderColor: '#e5e7eb' },
